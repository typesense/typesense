...
TEST(RaftServerTest, ResolveNodesConfigWithHostNames) {
    ASSERT_EQ("127.0.0.1:8107:8108,127.0.0.1:7107:7108,127.0.0.1:6107:6108",
              raft_config::resolve_node_hosts("127.0.0.1:8107:8108,127.0.0.1:7107:7108,127.0.0.1:6107:6108"));

    // Test localhost resolution - should accept either IPv4 or IPv6
    std::string localhost_result1 = raft_config::resolve_node_hosts("localhost:8107:8108,localhost:7107:7108,localhost:6107:6108");
    ...
    std::string localhost_result2 = raft_config::resolve_node_hosts("localhost:8107:8108localhost:7107:7108,localhost:6107:6108");
    ASSERT_TRUE(matches_either_ip_version(
        localhost_result2,
    ...
    ASSERT_EQ("",
              raft_config::resolve_node_hosts("typesense-node-2.typesense-service.typesense-"
...
TEST(RaftServerTest, ResolveNodesConfigWithIPv6) {
    ...
    ASSERT_EQ("[2001:db8::1]:8107:8108,[2001:db8::2]:7107:7108",
              raft_config::resolve_node_hosts("[2001:db8::1]:8107:8108,[2001:db8::2]:7107:7108"));

    // IPv6 with IPv4 mixed
    ASSERT_EQ("[2001:db8::1]:8107:8108,127.0.0.1:7107:7108",
              raft_config::resolve_node_hosts("[2001:db8::1]:8107:8108,127.0.0.1:7107:7108"));

    // IPv6 localhost
    ASSERT_EQ("[::1]:8107:8108",
              raft_config::resolve_node_hosts("[::1]:8107:8108"));

    // Malformed IPv6 inputs should be passed through unchanged
    ASSERT_EQ("[2001:db8::1:8107:8108",  // Missing closing bracket
              raft_config::resolve_node_hosts("[2001:db8::1:8107:8108"));

    // IPv6 with zone index
    ASSERT_EQ("[fe80::1%eth0]:8107:8108",
              raft_config::resolve_node_hosts("[fe80::1%eth0]:8107:8108"));

    // Test with real IPv6 hostname resolution - need to skip if resolution fails
    std::string ipv6_result = raft_config::resolve_node_hosts("ipv6.test-ipv6.com:8107:8108");
...
TEST(Hostname2IPStrTest, IPAddresses) {
    ...
    ASSERT_EQ("127.0.0.1", raft_config::hostname2ipstr("127.0.0.1"));
    ASSERT_EQ("192.168.1.1", raft_config::hostname2ipstr("192.168.1.1"));

    // Test IPv6 addresses - should return unchanged if already in brackets
    ASSERT_EQ("[::1]", raft_config::hostname2ipstr("[::1]"));
    ASSERT_EQ("[2001:db8::1]", raft_config::hostname2ipstr("[2001:db8::1]"));
}

TEST(Hostname2IPStrTest, Localhost) {
    std::string result = raft_config::hostname2ipstr("localhost");
...
TEST(Hostname2IPStrTest, PublicHostnames) {
    ...
    std::string ipv4_result = raft_config::hostname2ipstr("ipv4.test-ipv6.com");
