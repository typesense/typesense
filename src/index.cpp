...
Option<bool> Index::search_all_candidates(const size_t num_search_fields,
                                          ...
                                          Topster* topster,
                                          spp::sparse_hash_map<uint64_t, std::set<uint32_t>>& groups_processed,
...
Option<bool> Index::run_search(search_args* search_params, const std::string& collection_name,
    ...
    return search(search_params->field_query_tokens,
           ...
           search_params->curated_group_kv_map,
...
void Index::collate_included_ids(const std::vector<token_t>& q_included_tokens,
                                 ...
                                 std::vector<std::vector<art_leaf*>> & searched_queries,
                                 size_t group_limit, spp::sparse_hash_map<uint64_t, Topster*>& curated_group_kv_map) const {
    ...
    for(const auto& pos_ids: included_ids_map) {
        ...
        for(const auto& index_seq_id: pos_ids.second) {
            ...
            if(group_limit == 0) {
                ...
                KV kv(0, distinct_id, 0, scores);
                curated_topster->add(&kv);
            } else {
                int64_t scores[3] = {0};
                KV kv(0, seq_id, -1, scores);

                if (curated_group_kv_map.find(distinct_id) == curated_group_kv_map.end()) {
                    Topster *g_topster = new Topster(group_limit);
                    g_topster->add(&kv);
                    curated_group_kv_map.insert({distinct_id, g_topster});
                } else {
                    curated_group_kv_map[distinct_id]->add(&kv);
                }
            }
        }
    }
...
Option<bool> Index::search(std::vector<query_tokens_t>& field_query_tokens, const std::vector<search_field_t>& the_fields,
    ...
    auto process_groups = [&](Topster* topster, uint32_t* results_ids, uint32_t results_ids_len, bool is_reverse=false) {
        //process groups
        if (group_limit != 0) {
            auto group_by_field_it_vec = get_group_by_field_iterators(group_by_fields, is_reverse);

            for(auto i = 0; i < results_ids_len; ++i) {
                auto seq_id = results_ids[i];
                uint64_t distinct_id = 1;

                for (auto &kv: group_by_field_it_vec) {
                    get_distinct_id(kv.it, seq_id, kv.is_array, group_missing_values, distinct_id, is_reverse);
                }
                ...
                if(topster->key_exists(distinct_id)) {
                    ...
                    if (!compute_sort_scores_op.ok()) {
                        LOG(INFO) << "Failed to compute sort scores for groups.";
                        break;
                    }

                    KV kv(0, seq_id, match_score_index, scores);

                    if (group_kv_map.find(distinct_id) == group_kv_map.end()) {
                        Topster *g_topster = new Topster(group_limit);
                        g_topster->add(&kv);
                        group_kv_map.insert({distinct_id, g_topster});
                    } else {
    ...
    if (!field_query_tokens[0].q_phrases.empty()) {
        auto do_phrase_search_op = do_phrase_search(num_search_fields, the_fields, field_query_tokens,
                                                    ...
                                                    curated_group_kv_map, group_missing_values,
        ...
        if (filter_result_iterator->approx_filter_ids_length == 0) {
            process_groups(topster, all_result_ids, all_result_ids_len);
    ...
    if (is_wildcard_non_phrase_query) {
        if(no_filters_provided && facets.empty() && curated_ids.empty() && vector_query.field_name.empty() &&
           ...
           sort_fields_std[0].order == sort_field_const::desc) {
            ...
            while (it.valid()) {
                ...
                KV kv(searched_queries.size(), distinct_id, match_score_index, scores);
                int ret = topster->add(&kv);

                if(group_limit != 0) {
                    groups_processed[distinct_id].insert(seq_id);
                }
            ...
            process_groups(topster, result_ids.data(), result_ids.size(), true);
            goto process_search_results;
        }

        collate_included_ids({}, included_ids_map, curated_topster, searched_queries, group_limit, curated_group_kv_map);
        ...
        if (!vector_query.field_name.empty()) {
            ...
            for (auto& dist_result : dist_results) {
                ...
                KV kv(searched_queries.size(), distinct_id, match_score_index, scores, std::move(references));
       ...
       auto do_synonym_search_op = do_synonym_search(the_fields, match_type, filter_tree_root, included_ids_map,
                                                     ...
                                                     group_by_fields, group_kv_map, group_missing_values, prioritize_exact_match, prioritize_token_position,
...
Option<bool> Index::fuzzy_search_fields(const std::vector<search_field_t>& the_fields,
                                        ...
                                        std::vector<std::vector<art_leaf*>> & searched_queries,
                                        tsl::htrie_map<char, token_leaf>& qtoken_set,
                                        Topster* topster, spp::sparse_hash_map<uint64_t, std::set<uint32_t>>& groups_processed,
                                        uint32_t*& all_result_ids, size_t & all_result_ids_len,
                                        const size_t group_limit, const std::vector<std::string>& group_by_fields,
                                        const bool group_missing_values,
                                        bool prioritize_exact_match,
...
Option<bool> Index::search_wildcard(filter_node_t const* const& filter_tree_root,
    ...
    spp::sparse_hash_map<uint64_t, std::set<uint64_t>> tgroups_processed[num_threads];
    ...
    for(size_t thread_id = 0; thread_id < num_threads &&
        ...
        topsters[thread_id] = new Topster(topster->MAX_SIZE);
        ...
        thread_pool->enqueue([this, &parent_search_begin, &parent_search_stop_ms, &parent_search_cutoff,
            ...
            for(size_t i = 0; i < batch_result->count; i++) {
                ...
                if(group_limit != 0) {
                    tgroups_processed[thread_id][distinct_id].insert(seq_id);
                }
    ...
    for(size_t thread_id = 0; thread_id < num_processed; thread_id++) {
        ...
        for(const auto& it : tgroups_processed[thread_id]) {
            for(const auto& i : it.second) {
                groups_processed[it.first].insert(i);
            }
        }
...
void Index::score_results(const std::vector<sort_by> & sort_fields, const uint16_t & query_index,
    ...
    KV kv(query_index, distinct_id, match_score_index, scores);
    int ret = topster->add(&kv);
    if(group_limit != 0) {
        groups_processed[distinct_id].insert(seq_id);
    }
