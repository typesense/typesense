...
#define RETURN_CIRCUIT_BREAKER                                        \
    if (std::chrono::duration_cast<std::chrono::milliseconds>(        \
            std::chrono::high_resolution_clock::now() - search_begin) \
            .count() > search_stop_ms) {                              \
        search_cutoff = true;                                         \
        return;                                                       \
    }

#define BREAK_CIRCUIT_BREAKER                                         \
    if (std::chrono::duration_cast<std::chrono::milliseconds>(        \
            std::chrono::high_resolution_clock::now() - search_begin) \
            .count() > search_stop_ms) {                              \
        search_cutoff = true;                                         \
...
struct token_posting_t {
    ...
    token_posting_t(uint32_t token_id,
                    const posting_list_t::iterator_t& posting)
...
Index::Index(const std::string& name,
    ...
    for (const auto& a_field : search_schema) {
        ...
        if (a_field.is_string()) {
            ...
            auto field_geo_index =
                new spp::sparse_hash_map<std::string, std::vector<uint32_t>>();
            geopoint_index.emplace(a_field.name, field_geo_index);

            if (!a_field.is_single_geopoint()) {
                spp::sparse_hash_map<uint32_t, int64_t*>* doc_to_geos =
                    new spp::sparse_hash_map<uint32_t, int64_t*>();
        ...
        if (a_field.sort) {
            if (a_field.type == field_types::STRING) {
                ...
                spp::sparse_hash_map<uint32_t, int64_t>* doc_to_score =
                    new spp::sparse_hash_map<uint32_t, int64_t>();
        ...
        if (a_field.num_dim) {
            auto hnsw_index =
                new hnsw_index_t(a_field.num_dim, 1024, a_field.vec_dist);
...
int64_t Index::get_points_from_doc(const nlohmann::json& document,
    ...
    if (document[default_sorting_field].is_number_float()) {
        ...
        points ^= ((points >> (std::numeric_limits<int32_t>::digits - 1)) |
                   INT32_MIN);
        points = -1 * (INT32_MAX - points);
    } else if (document[default_sorting_field].is_string()) {
        // not much value in supporting default sorting field as string, so we
        // will just dummy it out
        points = 0;
    } else {
...
void Index::compute_token_offsets_facets(
    index_record& record,
    const tsl::htrie_map<char, field>& search_schema,
    ...
    for (const auto& the_field : search_schema) {
        ...
        // non-string, non-geo faceted field should be indexed as faceted string
        // field as well
        if (the_field.facet && !the_field.is_string() &&
            !the_field.is_geopoint()) {
            if (the_field.is_array()) {
                ...
                tokenize_string_array_with_facets(
                    strings, is_facet, the_field, local_symbols_to_index,
                    local_token_separators, offset_facet_hashes.offsets,
                    offset_facet_hashes.facet_hashes);
            } else {
                std::string text;
        ...
        if (the_field.is_string()) {
            if (the_field.type == field_types::STRING) {
                tokenize_string_with_facets(document[field_name], is_facet,
                                            the_field, local_symbols_to_index,
                                            local_token_separators,
                                            offset_facet_hashes.offsets,
                                            offset_facet_hashes.facet_hashes);
            } else {
                tokenize_string_array_with_facets(
                    document[field_name], is_facet, the_field,
        ...
        if (!offset_facet_hashes.offsets.empty() ||
            !offset_facet_hashes.facet_hashes.empty()) {
            record.field_index.emplace(field_name,
                                       std::move(offset_facet_hashes));
        }
    }
}

bool doc_contains_field(const nlohmann::json& doc,
                        const field& a_field,
                        const tsl::htrie_map<char, field>& search_schema) {
    ...
    // check for a nested field, e.g. `foo.bar.baz` indexed but `foo.bar`
    // present in schema
    if (a_field.is_object()) {
        ...
        for (auto kv = prefix_it.first; kv != prefix_it.second; kv++) {
            ...
            bool is_child_field =
                (nested_field_name.size() > a_field.name.size() &&
...
bool validate_object_field(nlohmann::json& doc, const field& a_field) {
    ...
    if (field_it != doc.end()) {
        if (a_field.type == field_types::OBJECT &&
            doc[a_field.name].is_object()) {
            return true;
        } else if (a_field.type == field_types::OBJECT_ARRAY &&
                   doc[a_field.name].is_array()) {
            return true;
        }
    ...
    if (a_field.type == field_types::OBJECT && obj->is_object()) {
        return true;
               ...
               (obj->is_array() || (has_array && obj->is_object()))) {
        return true;
    }
...
Option<uint32_t> Index::validate_index_in_memory(
    ...
    bool missing_default_sort_field =
        (!default_sorting_field.empty() &&
         document.count(default_sorting_field) == 0);

    if ((op != UPDATE && op != EMPLACE) && missing_default_sort_field) {
        return Option<>(400,
                        "Field `" + default_sorting_field +
                            "` has been declared as a default sorting field, "
    ...
    for (const auto& a_field : search_schema) {
        ...
        if ((a_field.optional || op == UPDATE || op == EMPLACE) &&
            document.count(field_name) == 0) {
            continue;
        }

        if (document.count(field_name) == 0) {
            return Option<>(400, "Field `" + field_name +
                                     "` has been declared in the schema, "
                                     "but is not found in the document.");
        }
        ...
        if (a_field.optional && document[field_name].is_null()) {
            ...
            if (op != UPDATE && op != EMPLACE) {
                // for updates, the erasure is done later since we need to keep
        ...
        if (a_field.type == field_types::STRING &&
            !document[field_name].is_string()) {
            Option<uint32_t> coerce_op = coerce_string(
                dirty_values, fallback_field_type, a_field, document,
            ...
            if (!document[field_name].is_number_integer()) {
                Option<uint32_t> coerce_op =
                    coerce_int32_t(dirty_values, a_field, document, field_name,
                ...
                if (!coerce_op.ok()) {
                   ...
                   !document[field_name].is_number_integer()) {
            Option<uint32_t> coerce_op =
                coerce_int64_t(dirty_values, a_field, document, field_name,
                               dummy_iter, false, array_ele_erased);
            if (!coerce_op.ok()) {
                return coerce_op;
            }
        } else if (a_field.type == field_types::FLOAT &&
                   !document[field_name].is_number()) {
            // using `is_number` allows integer to be passed to a float field
            Option<uint32_t> coerce_op =
                coerce_float(dirty_values, a_field, document, field_name,
                             dummy_iter, false, array_ele_erased);
            if (!coerce_op.ok()) {
                return coerce_op;
            }
        } else if (a_field.type == field_types::BOOL &&
                   !document[field_name].is_boolean()) {
            Option<uint32_t> coerce_op =
                coerce_bool(dirty_values, a_field, document, field_name,
                            dummy_iter, false, array_ele_erased);
            if (!coerce_op.ok()) {
                return coerce_op;
            }
            ...
            if (!document[field_name].is_array() ||
                document[field_name].size() != 2) {
                return Option<>(400,
                                "Field `" + field_name +
                                    "` must be a 2 element array: [lat, lng].");
            }

            if (!(document[field_name][0].is_number() &&
                  document[field_name][1].is_number())) {
                // one or more elements is not an number, try to coerce
                Option<uint32_t> coerce_op =
                    coerce_geopoint(dirty_values, a_field, document, field_name,
                                    dummy_iter, false, array_ele_erased);
                if (!coerce_op.ok()) {
                    return coerce_op;
                }
            ...
            if (!document[field_name].is_array()) {
                if (a_field.optional &&
                    (dirty_values == DIRTY_VALUES::DROP ||
                     dirty_values == DIRTY_VALUES::COERCE_OR_DROP)) {
                    document.erase(field_name);
                    continue;
                } else {
                    return Option<>(
                        400, "Field `" + field_name + "` must be an array.");
                }
            }
            ...
            for (; it != document[field_name].end();) {
                ...
                if (a_field.type == field_types::STRING_ARRAY &&
                    !item.is_string()) {
                    Option<uint32_t> coerce_op = coerce_string(
                        dirty_values, fallback_field_type, a_field, document,
                        field_name, it, true, array_ele_erased);
                    if (!coerce_op.ok()) {
                        return coerce_op;
                    }
                    ...
                    Option<uint32_t> coerce_op =
                        coerce_int32_t(dirty_values, a_field, document,
                                       field_name, it, true, array_ele_erased);
                    if (!coerce_op.ok()) {
                        return coerce_op;
                    }
                } else if (a_field.type == field_types::INT64_ARRAY &&
                           !item.is_number_integer()) {
                    Option<uint32_t> coerce_op =
                        coerce_int64_t(dirty_values, a_field, document,
                                       field_name, it, true, array_ele_erased);
                    if (!coerce_op.ok()) {
                        return coerce_op;
                    }
                } else if (a_field.type == field_types::FLOAT_ARRAY &&
                           !item.is_number()) {
                    // we check for `is_number` to allow whole numbers to be
                    // passed into float fields
                    Option<uint32_t> coerce_op =
                        coerce_float(dirty_values, a_field, document,
                                     field_name, it, true, array_ele_erased);
                    if (!coerce_op.ok()) {
                        return coerce_op;
                    }
                } else if (a_field.type == field_types::BOOL_ARRAY &&
                           !item.is_boolean()) {
                    Option<uint32_t> coerce_op =
                        coerce_bool(dirty_values, a_field, document, field_name,
                    ...
                    if (!item.is_array() || item.size() != 2) {
                        return Option<>(400, "Field `" + field_name +
                                                 "` must contain 2 element "
                                                 "arrays: [ [lat, lng],... ].");
                    }

                    if (!(item[0].is_number() && item[1].is_number())) {
                        // one or more elements is not an number, try to coerce
                        Option<uint32_t> coerce_op = coerce_geopoint(
                            dirty_values, a_field, document, field_name, it,
                            true, array_ele_erased);
                        if (!coerce_op.ok()) {
                            return coerce_op;
                        }
                    }
...
void Index::validate_and_preprocess(
    Index* index,
    std::vector<index_record>& iter_batch,
    const size_t batch_start_index,
    ...
    for (size_t i = 0; i < batch_size; i++) {
        ...
        try {
            ...
            if (do_validation) {
                Option<uint32_t> validation_op = validate_index_in_memory(
                    index_rec.doc, index_rec.seq_id, default_sorting_field,
                    search_schema, index_rec.operation, fallback_field_type,
                    index_rec.dirty_values);

                if (!validation_op.ok()) {
                    index_rec.index_failure(validation_op.code(),
                                            validation_op.error());
                    continue;
                }
            }
            ...
            if (index_rec.is_update) {
                ...
                get_doc_changes(index_rec.operation, index_rec.doc,
                                index_rec.old_doc, index_rec.new_doc,
                                index_rec.del_doc);
                scrub_reindex_doc(search_schema, index_rec.doc,
                                  index_rec.del_doc, index_rec.old_doc);
            }

            compute_token_offsets_facets(index_rec, search_schema,
                                         token_separators, symbols_to_index);

            int64_t points = 0;

            if (index_rec.doc.count(default_sorting_field) == 0) {
                auto default_sorting_field_it =
                    index->sort_index.find(default_sorting_field);
                if (default_sorting_field_it != index->sort_index.end()) {
                    auto seq_id_it = default_sorting_field_it->second->find(
                        index_rec.seq_id);
                ...
                points =
                    get_points_from_doc(index_rec.doc, default_sorting_field);
            }
...
size_t Index::batch_memory_index(
    Index* index,
    std::vector<index_record>& iter_batch,
    const std::string& default_sorting_field,
    const tsl::htrie_map<char, field>& search_schema,
    ...
    for (size_t thread_id = 0;
         thread_id < num_threads && batch_index < iter_batch.size();
         thread_id++) {
        ...
        index->thread_pool->enqueue([&, batch_index, batch_len]() {
            ...
            validate_and_preprocess(index, iter_batch, batch_index, batch_len,
                                    default_sorting_field, search_schema,
                                    fallback_field_type, token_separators,
                                    symbols_to_index, do_validation);
...

    {
        ...
        cv_process.wait(lock_process,
                        [&]() { return num_processed == num_queued; });
    }
    ...
    for (const auto& field_name : found_fields) {
        ...
        index->thread_pool->enqueue([&]() {
            ...
            const field& f = (field_name == "id")
                                 ...
                                 : search_schema.at(field_name);
            try {
                index->index_field_in_memory(f, iter_batch);
            } catch (std::exception& e) {
                LOG(ERROR) << "Unhandled Typesense error: " << e.what();
                for (auto& record : iter_batch) {
                    record.index_failure(500,
                                         "Unhandled Typesense error in index "
                                         "batch, check logs for details.");
                }
            }
...
void Index::index_field_in_memory(const field& afield,
                                  std::vector<index_record>& iter_batch) {
    ...
    if (afield.is_string() || non_string_facet_field) {
        std::unordered_map<std::string, std::vector<art_document>>
            token_to_doc_offsets;
        ...
        for (const auto& record : iter_batch) {
            ...
            if (afield.facet) {
                ...
                fhashvalues.hashes =
                    new uint64_t[field_index_it->second.facet_hashes.size()];

                for (size_t i = 0;
                     i < field_index_it->second.facet_hashes.size(); i++) {
                    fhashvalues.hashes[i] =
                        field_index_it->second.facet_hashes[i];
                }

                auto& facet_dim_index =
                    facet_index_v3[afield.name][seq_id % ARRAY_FACET_DIM];
                if (facet_dim_index == nullptr) {
                    LOG(ERROR)
                        << "Error, facet index not initialized for field "
                        << afield.name;
                } else {
            ...
            for (auto& token_offsets : field_index_it->second.offsets) {
                token_to_doc_offsets[token_offsets.first].emplace_back(
                    seq_id, record.points, token_offsets.second);

                if (afield.infix) {
                    auto strhash =
                        StringUtils::hash_wy(token_offsets.first.c_str(),
                                             token_offsets.first.size());
        ...
        for (auto& token_to_doc : token_to_doc_offsets) {
            ...
            int key_len =
                (int)token.length() + 1;  // for the terminating \0 char
    ...
    if (!afield.is_string()) {
        if (afield.type == field_types::INT32) {
            ...
            iterate_and_index_numerical_field(
                iter_batch, afield,
            ...
            iterate_and_index_numerical_field(
                ...
                [&afield, geo_index](const index_record& record,
                                     uint32_t seq_id) {
                    const std::vector<double>& latlong =
                        record.doc[afield.name];

                    S2RegionTermIndexer::Options options;
                    options.set_index_contains_points_only(true);
                    S2RegionTermIndexer indexer(options);
                    S2Point point =
                        S2LatLng::FromDegrees(latlong[0], latlong[1]).ToPoint();
            ...
            iterate_and_index_numerical_field(
                iter_batch, afield,
                [&afield, &geo_array_index = geo_array_index, geo_index](
                    const index_record& record, uint32_t seq_id) {
                    const std::vector<std::vector<double>>& latlongs =
                        record.doc[afield.name];
                    S2RegionTermIndexer::Options options;
                    options.set_index_contains_points_only(true);
                    S2RegionTermIndexer indexer(options);
                    ...
                    for (size_t li = 0; li < latlongs.size(); li++) {
                        ...
                        S2Point point =
                            S2LatLng::FromDegrees(latlong[0], latlong[1])
                                .ToPoint();
                        ...
                        int64_t packed_latlong =
                            GeoPoint::pack_lat_lng(latlong[0], latlong[1]);
                        packed_latlongs[li + 1] = packed_latlong;
                    }

                    geo_array_index.at(afield.name)
                        ->emplace(seq_id, packed_latlongs);
                });
            ...
            iterate_and_index_numerical_field(
                iter_batch, afield,
                [&afield, num_tree, &vector_index = vector_index](
                    const index_record& record, uint32_t seq_id) {
                    for (size_t arr_i = 0;
                         arr_i < record.doc[afield.name].size(); arr_i++) {
                    ...
                    if (afield.type == field_types::FLOAT_ARRAY &&
                        ...
                        size_t curr_ele_count =
                            vec_index->getCurrentElementCount();
                        if (curr_ele_count == vec_index->getMaxElements()) {
                            vec_index->resizeIndex(curr_ele_count * 1.3);
                        }

                        const std::vector<float>& float_vals =
                            record.doc[afield.name].get<std::vector<float>>();
                        if (afield.vec_dist == cosine) {
                            std::vector<float> normalized_vals(afield.num_dim);
                            hnsw_index_t::normalize_vector(float_vals,
                                                           normalized_vals);
                            vector_index[afield.name]->vecdex->addPoint(
                                normalized_vals.data(), (size_t)seq_id);
                        } else {
                            vector_index[afield.name]->vecdex->addPoint(
                                float_vals.data(), (size_t)seq_id);
                        }
                    }
                });
        }

        // add numerical values automatically into sort index if sorting is
        // enabled
        if (afield.is_num_sortable() &&
            afield.type != field_types::GEOPOINT_ARRAY) {
            spp::sparse_hash_map<uint32_t, int64_t>* doc_to_score =
                sort_index.at(afield.name);
            ...
            for (const auto& record : iter_batch) {
                ...
                if (is_integer) {
                    doc_to_score->emplace(seq_id,
                                          document[afield.name].get<int64_t>());
                } else if (is_float) {
                    int64_t ifloat =
                        float_to_int64_t(document[afield.name].get<float>());
                    doc_to_score->emplace(seq_id, ifloat);
                } else if (is_bool) {
                    doc_to_score->emplace(
                        seq_id, (int64_t)document[afield.name].get<bool>());
                } else if (is_geopoint) {
...
void Index::insert_doc(
    ...
    for (auto& kv : token_to_offsets) {
        ...
        int key_len =
            (int)kv.first.length() + 1;  // for the terminating \0 char
...
uint64_t Index::facet_token_hash(const field& a_field,
    ...
    if (a_field.is_float()) {
        ...
        reinterpret_cast<float&>(hash) =
            f;  // store as int without loss of precision
...
void Index::tokenize_string_with_facets(
...
void Index::tokenize_string_array_with_facets(
    ...
    for (size_t array_index = 0; array_index < strings.size(); array_index++) {
        ...
        std::set<std::string>
            token_set;  // required to deal with repeating tokens
        ...
        while (tokenizer.next(token, token_index)) {
            ...
            if (is_facet) {
                ...
                if (token_index == 0) {
                    ...
                    facet_hash =
                        StringUtils::hash_combine(facet_hash, token_hash);
                }
            }
        }
        ...
        for (auto& the_token : token_set) {
            // repeat last element to indicate end of offsets for this array
            // index
            token_to_offsets[the_token].push_back(
                token_to_offsets[the_token].back());
...
void Index::compute_facet_stats(facet& a_facet,
                                uint64_t raw_value,
                                const std::string& field_type) {
    if (field_type == field_types::INT32 ||
        field_type == field_types::INT32_ARRAY) {
        ...
        a_facet.stats.fvcount++;
               ...
               field_type == field_types::INT64_ARRAY) {
        ...
        a_facet.stats.fvsum += val;
        a_facet.stats.fvcount++;
    } else if (field_type == field_types::FLOAT ||
               field_type == field_types::FLOAT_ARRAY) {
...
void Index::do_facets(std::vector<facet>& facets,
    ...
    for (size_t findex = 0; findex < facets.size(); findex++) {
        ...
        const bool should_compute_stats =
            facet_infos[findex].should_compute_stats;

        const auto& field_facet_mapping_it =
            facet_index_v3.find(a_facet.field_name);
        ...
        for (size_t i = 0; i < results_size; i++) {
            ...
            const auto& facet_hashes_it =
                field_facet_mapping[doc_seq_id % ARRAY_FACET_DIM]->find(
                    doc_seq_id);

            if (facet_hashes_it ==
                field_facet_mapping[doc_seq_id % ARRAY_FACET_DIM]->end()) {
                continue;
            }

            const auto& facet_hashes = facet_hashes_it->second;
            const uint64_t distinct_id =
                group_limit ? get_distinct_id(group_by_fields, doc_seq_id) : 0;
            ...
            for (size_t j = 0; j < facet_hashes.size(); j++) {
                ...
                if (!use_facet_query ||
                    fquery_hashes.find(fhash) != fquery_hashes.end()) {
                    facet_count_t& facet_count = a_facet.result_map[fhash];

                    // LOG(INFO) << "field: " << a_facet.field_name << ", doc
                    // id: " << doc_seq_id << ", hash: " <<  fhash;
...
void Index::search_all_candidates(
    ...
    /*if(!token_candidates_vec.empty()) {
        LOG(INFO) << "Prefix candidates size: " <<
    token_candidates_vec.back().candidates.size(); LOG(INFO) << "max_candidates:
    " << max_candidates; LOG(INFO) << "token_candidates_vec.size(): " <<
    token_candidates_vec.size();
    }*/
    ...
    // escape hatch to prevent too much looping but subject to being overriden
    // explicitly via `max_candidates`
    long long combination_limit =
        (num_search_fields == 1 && prefixes[0])
            ? max_candidates
    ...
    for (long long n = 0; n < N && n < combination_limit; ++n) {
        ...
        uint64 qhash;
        uint32_t total_cost =
            next_suggestion2(token_candidates_vec, n, query_suggestion, qhash);
        ...
        LOG(INFO) << "query: " << fullq.str() << ", total_cost: " << total_cost
                  << ", all_result_ids_len: " << all_result_ids_len << ",
        bufsiz: " << id_buff.size();*/
        ...
        // LOG(INFO) << "field_num_results: " << field_num_results << ",
        // typo_tokens_threshold: " << typo_tokens_threshold;

        search_across_fields(
            query_suggestion, num_typos, prefixes, the_fields,
            num_search_fields, sort_fields, topster, groups_processed,
            searched_queries, qtoken_set, group_limit, group_by_fields,
            prioritize_exact_match, prioritize_token_position, filter_ids,
            filter_ids_length, total_cost, syn_orig_num_tokens,
            exclude_token_ids, exclude_token_ids_size, sort_order, field_values,
            geopoint_indices, id_buff, all_result_ids, all_result_ids_len);
...
void Index::search_candidates(
    ...
    std::set<uint64>& query_hashes,
    std::vector<uint32_t>& id_buff) const {
    auto product = [](long long a, token_candidates& b) {
        return a * b.candidates.size();
    };
    long long int N = std::accumulate(token_candidates_vec.begin(),
                                      token_candidates_vec.end(), 1LL, product);

    int sort_order[3];  // 1 or -1 based on DESC or ASC respectively
    std::array<spp::sparse_hash_map<uint32_t, int64_t>*, 3> field_values;
    std::vector<size_t> geopoint_indices;

    populate_sort_mapping(sort_order, geopoint_indices, sort_fields,
                          field_values);

    // escape hatch to prevent too much looping
    size_t combination_limit = exhaustive_search ? Index::COMBINATION_MAX_LIMIT
                                                 : Index::COMBINATION_MIN_LIMIT;

    for (long long n = 0; n < N && n < combination_limit; ++n) {
        RETURN_CIRCUIT_BREAKER

        // every element in `query_suggestion` contains a token and its
        // associated hits
        ...
        uint32_t total_cost = next_suggestion(
            token_candidates_vec, n, actual_query_suggestion, query_suggestion,
            syn_orig_num_tokens, token_bits, qhash);
        ...
        // LOG(INFO) << "field_num_results: " << field_num_results << ",
        // typo_tokens_threshold: " << typo_tokens_threshold; LOG(INFO) << "n: "
        // << n;

        /*std::stringstream fullq;
        for(const auto& qleaf : actual_query_suggestion) {
            std::string qtok(reinterpret_cast<char*>(qleaf->key),qleaf->key_len
        - 1); fullq << qtok << " ";
        }
        LOG(INFO) << "field: " << size_t(field_id) << ", query: " << fullq.str()
        << ", total_cost: " << total_cost;*/

        // Prepare excluded document IDs that we can later remove from the
        // result set
        uint32_t* excluded_result_ids = nullptr;
        size_t excluded_result_ids_size = ArrayUtils::or_scalar(
            exclude_token_ids, exclude_token_ids_size, &curated_ids[0],
        ...
        result_iter_state_t iter_state(excluded_result_ids,
                                       excluded_result_ids_size, filter_ids,
                                       filter_ids_length);
        ...
        if (total_cost == 0 &&
            query_suggestion.size() == query_tokens.size() == 1) {
        ...
        std::vector<spp::sparse_hash_set<uint64_t>> groups_processed_vec(
            concurrency);
            ...
            posting_t::block_intersector_t(posting_lists, iter_state,
                ...
                .intersect(
                    [&](uint32_t seq_id,
                        ...
                        size_t index) {
                        score_results(
                            sort_fields, searched_queries.size(), field_id,
                            field_is_array, total_cost, topsters[index],
                            query_suggestion, groups_processed_vec[index],
        ...
        for (size_t i = 0; i < concurrency; i++) {
            ...
            if (!result_id_vecs[i].empty()) {
                if (exhaustive_search) {
                    id_buff.insert(id_buff.end(), result_id_vecs[i].begin(),
                                   result_id_vecs[i].end());
                } else {
                    uint32_t* new_all_result_ids = nullptr;
                    all_result_ids_len = ArrayUtils::or_scalar(
                        *all_result_ids, all_result_ids_len,
                        &result_id_vecs[i][0], result_id_vecs[i].size(),
                        &new_all_result_ids);
                ...
                if (topster != nullptr) {
                    // topster is null when used by overrides which requires
                    // only IDs but not actual processing
                    aggregate_topster(topster, topsters[i]);
                    groups_processed.insert(groups_processed_vec[i].begin(),
                                            groups_processed_vec[i].end());
                }
            }
        ...
        if (id_buff.size() > 100000) {
            ...
            id_buff.erase(std::unique(id_buff.begin(), id_buff.end()),
                          id_buff.end());

            uint32_t* new_all_result_ids = nullptr;
            all_result_ids_len = ArrayUtils::or_scalar(
                *all_result_ids, all_result_ids_len, &id_buff[0],
...
//                 filter_ids_length = ArrayUtils::and_scalar(filter_ids,
//                 filter_ids_length, &result_ids[0],
//                                                            result_ids.size(),
//                                                            &filtered_results);
//                 delete [] filter_ids;
...
//         ||
//                                 numerical_index.count(a_filter.field_name) !=
//                                 0 ||
//                                 geopoint_index.count(a_filter.field_name) !=
//                                 0;
...
//                 if(a_filter.comparators[fi] == RANGE_INCLUSIVE && fi+1 <
//                 a_filter.values.size()) {
//                     const std::string& next_filter_value =
//                     a_filter.values[fi+1]; int64_t range_end_value =
//                     (int64_t) std::stol(next_filter_value);
//                     num_tree->range_inclusive_search(value, range_end_value,
//                     &result_ids, result_ids_len); fi++;
//                 } else {
//                     num_tree->search(a_filter.comparators[fi], value,
//                     &result_ids, result_ids_len);
//                 }
//             }
...
//                     const std::string& next_filter_value =
//                     a_filter.values[fi+1]; int64_t range_end_value =
//                     float_to_int64_t((float)
//                     std::atof(next_filter_value.c_str()));
//                     num_tree->range_inclusive_search(float_int64,
//                     range_end_value, &result_ids, result_ids_len); fi++;
//                 } else {
...
//                     num_tree->search(EQUALS, bool_int64, &to_exclude_ids,
//                     to_exclude_ids_len);
...
//                     excluded_ids_len = ArrayUtils::exclude_scalar(all_ids,
//                     all_ids_size, to_exclude_ids,
//                                                                   to_exclude_ids_len,
//                                                                   &excluded_ids);
...
//                     result_ids_len = ArrayUtils::or_scalar(result_ids,
//                     result_ids_len,
//                                                            excluded_ids,
//                                                            excluded_ids_len,
//                                                            &out);
...
//                     delete [] excluded_ids;
//                 } else {
//                     num_tree->search(a_filter.comparators[value_index],
//                     bool_int64, &result_ids, result_ids_len);
//                 }
...
//                 x, y, 2, km (or) list of points

//                 bool is_polygon =
//                 StringUtils::is_float(filter_value_parts.back()); S2Region*
//                 query_region;
...
//                     size_t(num_verts); point_index++) {
//                         double lat = std::stod(filter_value_parts[point_index
//                         * 2]); double lon =
//                         std::stod(filter_value_parts[point_index * 2 + 1]);
//                         S2Point vertex = S2LatLng::FromDegrees(lat,
//                         lon).ToPoint(); vertices.emplace_back(vertex);
//                     }

//                     auto loop = new S2Loop(vertices, S2Debug::DISABLE);
//                     loop->Normalize(); // if loop is not CCW but CW, change
//                     to CCW.

//                     S2Error error;
//                     if (loop->FindValidationError(&error)) {
//                         LOG(ERROR) << "Query vertex is bad, skipping. Error:
//                         " << error; delete loop; continue;
//                     } else {
//                         query_region = loop;
//                     }
//                 } else {
...
//                     S1Angle query_radius =
//                     S1Angle::Radians(S2Earth::MetersToRadians(radius));
//                     double query_lat = std::stod(filter_value_parts[0]);
//                     double query_lng = std::stod(filter_value_parts[1]);
//                     S2Point center = S2LatLng::FromDegrees(query_lat,
//                     query_lng).ToPoint(); query_region = new S2Cap(center,
//                     query_radius);
//                 }
...
//                         geo_result_ids.insert(geo_result_ids.end(),
//                         ids_it->second.begin(), ids_it->second.end());
//                     }
//                 }

//                 gfx::timsort(geo_result_ids.begin(), geo_result_ids.end());
//                 geo_result_ids.erase(std::unique( geo_result_ids.begin(),
//                 geo_result_ids.end() ), geo_result_ids.end());

//                 // `geo_result_ids` will contain all IDs that are within
//                 approximately within query radius
//                 // we still need to do another round of exact filtering on
//                 them

//                 std::vector<uint32_t> exact_geo_result_ids;

//                 if(f.is_single_geopoint()) {
//                     spp::sparse_hash_map<uint32_t, int64_t>* sort_field_index
//                     = sort_index.at(f.name);

//                     for(auto result_id: geo_result_ids) {
//                         // no need to check for existence of `result_id`
//                         because of indexer based pre-filtering above int64_t
//                         lat_lng = sort_field_index->at(result_id); S2LatLng
//                         s2_lat_lng; GeoPoint::unpack_lat_lng(lat_lng,
//                         s2_lat_lng); if
//                         (query_region->Contains(s2_lat_lng.ToPoint())) {
//                             exact_geo_result_ids.push_back(result_id);
//                         }
//                     }
//                 } else {
//                     spp::sparse_hash_map<uint32_t, int64_t*>* geo_field_index
//                     = geo_array_index.at(f.name);
...
//                 uint32_t *out = nullptr;
//                 result_ids_len =
//                 ArrayUtils::or_scalar(&exact_geo_result_ids[0],
//                 exact_geo_result_ids.size(),
//                                                        result_ids,
//                                                        result_ids_len, &out);
...
//             // aggregates IDs across array of filter values and reduces
//             excessive ORing std::vector<uint32_t> f_id_buff;

//             for(const std::string & filter_value: a_filter.values) {
//                 std::vector<void*> posting_lists;

//                 // there could be multiple tokens in a filter value, which we
//                 have to treat as ANDs
...
//                     art_leaf* leaf = (art_leaf *) art_search(t, (const
//                     unsigned char*) str_token.c_str(),
//                                                              str_token.length()+1);
//                     if(leaf == nullptr) {
//                         continue;
//                     }
...
//                 if(a_filter.comparators[0] == EQUALS ||
...
//                     uint32_t* exact_str_ids = new
//                     uint32_t[result_id_vec.size()]; size_t exact_str_ids_size
//                     = 0; std::unique_ptr<uint32_t[]>
//                     exact_str_ids_guard(exact_str_ids);

//                     posting_t::get_exact_matches(posting_lists, f.is_array(),
...
//                     f_id_buff.erase(std::unique( f_id_buff.begin(),
//                     f_id_buff.end() ), f_id_buff.end());

//                     uint32_t* out = nullptr;
//                     or_ids_size = ArrayUtils::or_scalar(or_ids, or_ids_size,
//                     f_id_buff.data(), f_id_buff.size(), &out); delete[]
//                     or_ids; or_ids = out;
//                     std::vector<uint32_t>().swap(f_id_buff); // clears out
//                     memory
//                 }
//             }
...
//                 uint32_t* out = nullptr;
//                 or_ids_size = ArrayUtils::or_scalar(or_ids, or_ids_size,
//                 f_id_buff.data(), f_id_buff.size(), &out); delete[] or_ids;
//                 or_ids = out;
//                 std::vector<uint32_t>().swap(f_id_buff); // clears out memory
//             }

//             if(a_filter.comparators[0] == NOT_EQUALS) {
//                 // exclude records from existing IDs (from previous filters
//                 or ALL records)
//                 // "not equals" can only be applied to the entire array so we
//                 can do the exclusion operations once here uint32_t*
//                 excluded_strt_ids = nullptr; size_t excluded_strt_size = 0;
...
//                 excluded_strt_size = ArrayUtils::exclude_scalar(result_ids,
//                 result_ids_len, or_ids,
//                                                                 or_ids_size,
//                                                                 &excluded_strt_ids);

//                 if(filter_ids == nullptr) {
//                     // means we had to uncompress `seq_ids` so need to free
//                     that delete [] result_ids;
//                 }
...
// void Index::do_filtering_with_lock(uint32_t*& filter_ids, uint32_t&
// filter_ids_length,
//                                    const std::vector<filter>& filters) const
//                                    {
//     std::shared_lock lock(mutex);
...
void Index::do_filtering2(uint32_t*& filter_ids,
                          uint32_t& filter_ids_length,
    ...
    if (a_filter.field_name == "id") {
        ...
        if (filter_ids_length == 0) {
            ...
            filter_ids_length = ArrayUtils::and_scalar(
                filter_ids, filter_ids_length, &result_ids[0],
    ...
    if (f.is_integer()) {
        ...
        for (size_t fi = 0; fi < a_filter.values.size(); fi++) {
            ...
            if (a_filter.comparators[fi] == RANGE_INCLUSIVE &&
                fi + 1 < a_filter.values.size()) {
                ...
                num_tree->search(a_filter.comparators[fi], value, &result_ids,
                                 result_ids_len);
            }
        }
    } else if (f.is_float()) {
        ...
        for (size_t fi = 0; fi < a_filter.values.size(); fi++) {
            ...
            if (a_filter.comparators[fi] == RANGE_INCLUSIVE &&
                ...
                const std::string& next_filter_value = a_filter.values[fi + 1];
                int64_t range_end_value = float_to_in64_t(
                    (float)std::atof(next_filter_value.c_str()));
                num_tree->range_inclusive_search(float_int64, range_end_value,
                                                 &result_ids, result_ids_len);
                fi++;
            } else {
                num_tree->search(a_filter.comparators[fi], float_int64,
        ...
        for (const std::string& filter_value : a_filter.values) {
            ...
            if (a_filter.comparators[value_index] == NOT_EQUALS) {
                ...
                excluded_ids_len = ArrayUtils::exclude_scalar(
                    all_ids, all_ids_size, to_exclude_ids, to_exclude_ids_len,
                    &excluded_ids);
                ...
                uint32_t* out = nullptr;
                result_ids_len =
                    ArrayUtils::or_scalar(result_ids, result_ids_len,
        ...
        for (const std::string& filter_value : a_filter.values) {
            ...
            if (is_polygon) {
                ...
                if (loop->FindValidationError(&error)) {
                    LOG(ERROR)
                        << "Query vertex is bad, skipping. Error: " << error;
                    delete loop;
                    continue;
                } else {
                    query_region = loop;
                ...
                double query_lng = std::stod(filter_value_parts[1]);
                S2Point center =
                    S2LatLng::FromDegrees(query_lat, query_lng).ToPoint();
            ...
            gfx::timsort(geo_result_ids.begin(), geo_result_ids.end());
            geo_result_ids.erase(
                std::unique(geo_result_ids.begin(), geo_result_ids.end()),
                geo_result_ids.end());

            // `geo_result_ids` will contain all IDs that are within
            // approximately within query radius we still need to do another
            // round of exact filtering on them
            ...
            if (f.is_single_geopoint()) {
                ...
                for (auto result_id : geo_result_ids) {
                    // no need to check for existence of `result_id` because of
                    // indexer based pre-filtering above
            ...
            uint32_t* out = nullptr;
            result_ids_len = ArrayUtils::or_scalar(
                &exact_geo_result_ids[0], exact_geo_result_ids.size(),
        ...
        for (const std::string& filter_value : a_filter.values) {
            ...
            // to treat as ANDs e.g. country: South Africa
            ...
            while (tokenizer.next(str_token, token_index)) {
                ...
                art_leaf* leaf = (art_leaf*)art_search(
                    t, (const unsigned char*)str_token.c_str(),
                    str_token.length() + 1);
                if (leaf == nullptr) {
                    continue;
                }
            ...
            // For NOT_EQUALS alone, it is okay for none of the results to match
            // prior to negation e.g. field:!= [RANDOM_NON_EXISTING_STRING]
            if (a_filter.comparators[0] != NOT_EQUALS &&
                posting_lists.size() != str_tokens.size()) {
                continue;
            }
            ...
            if (a_filter.comparators[0] == NOT_EQUALS) {
                // exclude records from existing IDs (from previous filters or
                // ALL records) upstream will guarantee that NOT_EQUALS is
                // placed right at the end of filters list
                ...
                excluded_strt_size = ArrayUtils::exclude_scalar(
                    ids, ids_size, strt_ids, strt_ids_size, &excluded_strt_ids);
                ...
                // Otherwise, we just ensure that given record contains tokens
                // in the filter query
                uint32_t* out = nullptr;
                ids_size = ArrayUtils::or_scalar(ids, ids_size, strt_ids,
                                                 strt_ids_size, &out);
                delete[] strt_ids;
                delete[] ids;
                ids = out;
...
void Index::recurse(uint32_t*& filter_ids,
    ...
    if (root->isOperator) {
        ...
        if (l_filter_ids != nullptr) {
            delete[] l_filter_ids;
        }
        if (r_filter_ids != nullptr) {
            delete[] r_filter_ids;
        }
        filter_ids = filtered_results;
...
void Index::do_filtering_with_lock(
    uint32_t*& filter_ids,
    uint32_t& filter_ids_length,
...
void Index::run_search(search_args* search_params) {
    search(search_params->field_query_tokens, search_params->search_fields,
           search_params->filter_tree_root, search_params->facets,
           search_params->facet_query, search_params->included_ids,
           search_params->excluded_ids, search_params->sort_fields_std,
           search_params->num_typos, search_params->topster,
           search_params->curated_topster, search_params->per_page,
           search_params->page, search_params->token_order,
           ...
           search_params->searched_queries, search_params->qtoken_set,
           search_params->raw_result_kvs, search_params->override_result_kvs,
           search_params->typo_tokens_threshold, search_params->group_limit,
           search_params->group_by_fields, search_params->default_sorting_field,
           search_params->prioritize_exact_match,
           search_params->prioritize_token_position,
           search_params->exhaustive_search, search_params->concurrency,
           search_params->search_cutoff_ms, search_params->min_len_1typo,
           search_params->min_len_2typo, search_params->max_candidates,
           search_params->infixes, search_params->max_extra_prefix,
           search_params->max_extra_suffix,
           search_params->facet_query_num_typos,
           search_params->filter_curated_hits, search_params->split_join_tokens,
           search_params->vector_query);
}

void Index::collate_included_ids(
    const std::vector<token_t>& q_included_tokens,
    ...
    for (const auto& pos_ids : included_ids_map) {
        ...
        for (const auto& index_seq_id : pos_ids.second) {
            ...
            uint64_t distinct_id =
                outer_pos;  // outer pos is the group distinct key
            uint64_t match_score =
                (64000 - outer_pos -
                 inner_pos);  // both outer pos and inner pos inside group
            ...
            uint32_t token_bits = 0;
            KV kv(0, searched_queries.size(), token_bits, seq_id, distinct_id,
                  0, scores);
...
bool Index::static_filter_query_eval(const override_t* override,
    ...
    if ((override->rule.match == override_t::MATCH_EXACT &&
...
bool Index::resolve_override(const std::vector<std::string>& rule_tokens,
                             const bool exact_rule_match,
    ...
    while (i < rule_tokens.size()) {
        if (rule_tokens[i].front() == '{' && rule_tokens[i].back() == '}') {
            ...
            field_names.emplace_back(
                rule_part.erase(0, 1).erase(rule_part.size() - 1));

            // skip until we find a non-placeholder token
            i++;

            while (i < rule_tokens.size() && (rule_tokens[i].front() == '{' &&
                                              rule_tokens[i].back() == '}')) {
            ...
            // `i` now points to either end of array or at a non-placeholder
            // rule token end of array: add remaining query tokens as matched
            // tokens non-placeholder: skip query tokens until it matches a rule
            // token

            while (j < query_tokens.size() &&
                   (i == rule_tokens.size() ||
                    rule_tokens[i] != query_tokens[j])) {
            ...
            for (size_t findex = 0; findex < field_names.size(); findex++) {
                ...
                bool slide_window =
                    (findex ==
                     0);  // fields following another field should match exactly
                std::vector<std::string> field_absorbed_tokens;
                resolved_override &= check_for_overrides(
                    token_order, field_name, slide_window, exact_rule_match,
                    matched_tokens, absorbed_tokens, field_absorbed_tokens);
            ...
            // rule token is not a placeholder, so we have to skip the query
            // tokens until it matches rule token
            while (j < query_tokens.size() &&
                   query_tokens[j] != rule_tokens[i]) {
                if (exact_rule_match) {
...
RETURN_EARLY:
    ...
    if (!resolved_override ||
        (exact_rule_match && query_tokens.size() != absorbed_tokens.size())) {
        return false;
    }
...
void Index::process_filter_overrides(
    ...
    for (auto& override : filter_overrides) {
        if (!override->rule.dynamic_query) {
            // Simple static filtering: add to filter_by and rewrite query if
            // needed. Check the original query and then the synonym variants
            // until a rule matches.
            bool resolved_override = static_filter_query_eval(
                override, query_tokens, filter_tree_root);
            ...
            if (resolved_override) {
                if (override->remove_matched_tokens) {
                    ...
                    std::set<std::string> rule_token_set(rule_tokens.begin(),
                                                         rule_tokens.end());
            ...
            // need to extract placeholder field names from the search query,
            // filter on them and rewrite query we will cover both original
            // query and synonyms
            ...
            bool exact_rule_match =
                override->rule.match == override_t::MATCH_EXACT;
            std::string filter_by_clause = override->filter_by;

            std::set<std::string> absorbed_tokens;
            bool resolved_override = resolve_override(
                rule_parts, exact_rule_match, query_tokens, token_order,
...
void Index::remove_matched_tokens(std::vector<std::string>& tokens,
                                  const std::set<std::string>& rule_token_set) {
...
bool Index::check_for_overrides(
    const token_ordering& token_order,
    const string& field_name,
    const bool slide_window,
    bool exact_rule_match,
    std::vector<std::string>& tokens,
    std::set<std::string>& absorbed_tokens,
    ...
    for (size_t window_len = tokens.size(); window_len > 0; window_len--) {
        for (size_t start_index = 0;
             start_index + window_len - 1 < tokens.size(); start_index++) {
            ...
            std::vector<sort_by> sort_fields;
            search_field(
                0, window_tokens, nullptr, 0, num_toks_dropped,
                field_it.value(), field_name, nullptr, 0, {}, sort_fields, -1,
                0, searched_queries, topster, groups_processed, &result_ids,
                result_ids_len, field_num_results, 0, group_by_fields, false, 4,
                query_hashes, token_order, false, 0, 0, false, -1, 3, 7, 4);
            ...
            if (result_ids_len != 0) {
                ...
                for (auto& w_token : window_tokens) {
                    art_leaf* leaf = (art_leaf*)art_search(
                        t, (const unsigned char*)w_token.value.c_str(),
                ...
                posting_t::get_exact_matches(
                    posting_lists, field_it.value().is_array(), result_ids,
                    result_ids_len, exact_strt_ids, exact_strt_size);
                ...
                if (exact_strt_size != 0) {
                    ...
                    std::vector<std::string> new_tokens;
                    for (size_t new_i = start_index; new_i < tokens.size();
                         new_i++) {
...
void Index::search_infix(const std::string& query,
    ...
    for (auto infix_set : infix_sets) {
        thread_pool->enqueue([infix_set, &leaves, search_tree, &query,
                              max_extra_prefix, max_extra_suffix,
                              &num_processed, &m_process, &cv_process,
                              &parent_search_begin, &parent_search_stop_ms,
                              &parent_search_cutoff]() {
            ...
            for (auto it = infix_set->begin(); it != infix_set->end(); it++) {
                ...
                if (start_index != std::string::npos &&
                    start_index <= max_extra_prefix &&
                    (key_buffer.size() - (start_index + query.size())) <=
                        max_extra_suffix) {
                    art_leaf* l = (art_leaf*)art_search(
                        search_tree, (const unsigned char*)key_buffer.c_str(),
                        key_buffer.size() + 1);
                    if (l != nullptr) {
                ...
                // check for search cutoff but only once every 2^10 docs to
                // reduce overhead
                if (((num_iterated + 1) % (1 << 12)) == 0) {
                    if (std::chrono::duration_cast<std::chrono::milliseconds>(
                            std::chrono::high_resolution_clock::now() -
                            search_begin)
                            .count() > op_search_stop_ms) {
                        search_cutoff = true;
                        break;
                    }
                }
    ...
    std::unique_lock<std::mutex> lock_process(m_process);
    cv_process.wait(lock_process,
                    [&]() { return num_processed == infix_sets.size(); });
    search_cutoff = parent_search_cutoff;
...
void Index::search(
    ...
    std::map<size_t, std::map<size_t, uint32_t>>
        included_ids_map;  // outer pos => inner pos => list of IDs
    std::vector<uint32_t> included_ids_vec;
    process_curated_ids(included_ids, excluded_ids, group_limit,
                        filter_curated_hits, filter_ids, filter_ids_length,
                        curated_ids, included_ids_map, included_ids_vec);

    std::vector<uint32_t> curated_ids_sorted(curated_ids.begin(),
                                             curated_ids.end());
    ...
    const size_t num_search_fields =
        std::min(the_fields.size(), (size_t)FIELD_LIMIT_NUM);
    ...
    handle_exclusion(num_search_fields, field_query_tokens, the_fields,
                     exclude_token_ids, exclude_token_ids_size);

    int sort_order[3];  // 1 or -1 based on DESC or ASC respectively
    std::array<spp::sparse_hash_map<uint32_t, int64_t>*, 3> field_values;
    std::vector<size_t> geopoint_indices;
    populate_sort_mapping(sort_order, geopoint_indices, sort_fields_std,
                          field_values);

    // Prepare excluded document IDs that we can later remove from the result
    // set
    uint32_t* excluded_result_ids = nullptr;
    size_t excluded_result_ids_size = ArrayUtils::or_scalar(
        exclude_token_ids, exclude_token_ids_size, &curated_ids_sorted[0],
        curated_ids_sorted.size(), &excluded_result_ids);

    auto is_wildcard_query =
        !field_query_tokens.empty() &&
        !field_query_tokens[0].q_include_tokens.empty() &&
        field_query_tokens[0].q_include_tokens[0].value == "*";

    // for phrase query, parser will set field_query_tokens to "*", need to
    // handle that
    if (is_wildcard_query) {
        const uint8_t field_id = (uint8_t)(FIELD_LIMIT_NUM - 0);
        bool no_filters_provided = (filters.empty() && filter_ids_length == 0);

        if (no_filters_provided && facets.empty() && curated_ids.empty() &&
            vector_query.field_name.empty() && sort_fields_std.size() == 1 &&
            sort_fields_std[0].name == sort_field_const::seq_id &&
            ...
            while (it.valid()) {
                ...
                KV kv(field_id, searched_queries.size(), 0, seq_id, distinct_id,
                      match_score_index, scores);
                topster->add(&kv);
        ...
        curate_filtered_ids(filter_tree_root, curated_ids, excluded_result_ids,
                            excluded_result_ids_size, filter_ids,
                            filter_ids_length, curated_ids_sorted);
        collate_included_ids({}, included_ids_map, curated_topster,
                             searched_queries);
        ...
        if (!vector_query.field_name.empty()) {
            ...
            if (field_vector_index->distance_type == cosine) {
                ...
                hnsw_index_t::normalize_vector(vector_query.values,
                                               normalized_q);
                dist_labels = field_vector_index->vecdex->searchKnnCloserFirst(
                    normalized_q.data(), k, filterFunctor);
            } else {
                dist_labels = field_vector_index->vecdex->searchKnnCloserFirst(
                    vector_query.values.data(), k, filterFunctor);
            }
            ...
            for (const auto& dist_label : dist_labels) {
                ...
                auto vec_dist_score =
                    (field_vector_index->distance_type == cosine)
                ...
                // LOG(INFO) << "SEQ_ID: " << seq_id << ", score: " <<
                // dist_label.first;

                KV kv(0, searched_queries.size(), 0, seq_id, distinct_id,
                      match_score_index, scores);
                topster->add(&kv);
            ...
            if (!nearest_ids.empty()) {
                uint32_t* new_all_result_ids = nullptr;
                all_result_ids_len = ArrayUtils::or_scalar(
                    all_result_ids, all_result_ids_len, nearest_ids.data(),
            ...
            search_wildcard(
                filter_tree_root, included_ids_map, sort_fields_std, topster,
                curated_topster, groups_processed, searched_queries,
                group_limit, group_by_fields, curated_ids, curated_ids_sorted,
                excluded_result_ids, excluded_result_ids_size, all_result_ids,
                all_result_ids_len, filter_ids, filter_ids_length, concurrency,
                sort_order, field_values, geopoint_indices);
        }
    } else {
        // Non-wildcard
        // In multi-field searches, a record can be matched across different
        // fields, so we use this for aggregation begin =
        // std::chrono::high_resolution_clock::now();
        ...
        std::vector<std::vector<token_t>> all_queries = {
            field_query_tokens[0].q_include_tokens};
        std::vector<std::vector<token_t>> q_pos_synonyms;
        std::vector<std::string> q_include_tokens;
        int syn_orig_num_tokens = -1;

        for (size_t j = 0; j < field_query_tokens[0].q_include_tokens.size();
             j++) {
            q_include_tokens.push_back(
                field_query_tokens[0].q_include_tokens[j].value);
        }
        synonym_index->synonym_reduction(q_include_tokens,
        ...
        if (split_join_tokens == always ||
            (all_result_ids_len == 0 && split_join_tokens == fallback)) {
            std::vector<std::vector<std::string>> space_resolved_queries;

            for (size_t i = 0; i < num_search_fields; i++) {
                std::vector<std::string> orig_q_include_tokens;
                for (auto& q_include_token :
                     field_query_tokens[i].q_include_tokens) {
                    orig_q_include_tokens.push_back(q_include_token.value);
                }

                resolve_space_as_typos(orig_q_include_tokens,
                                       the_fields[i].name,
            ...
            if (!space_resolved_queries.empty()) {
                ...
                for (size_t j = 0; j < resolved_query.size(); j++) {
                    ...
                    resolved_tokens.emplace_back(
                        j, space_resolved_queries[0][j], is_prefix,
        ...
        do_synonym_search(
            the_fields, filter_tree_root, included_ids_map, sort_fields_std,
            curated_topster, token_order, 0, group_limit, group_by_fields,
            prioritize_exact_match, prioritize_token_position,
            exhaustive_search, concurrency, prefixes, min_len_1typo,
            min_len_2typo, max_candidates, curated_ids, curated_ids_sorted,
            excluded_result_ids, excluded_result_ids_size, topster,
            q_pos_synonyms, syn_orig_num_tokens, groups_processed,
            searched_queries, all_result_ids, all_result_ids_len, filter_ids,
            filter_ids_length, query_hashes, sort_order, field_values,
            geopoint_indices, qtoken_set);
        ...
        if (all_result_ids_len < drop_tokens_threshold) {
            for (size_t qi = 0; qi < all_queries.size(); qi++) {
                ...
                while (exhaustive_search ||
                       all_result_ids_len < drop_tokens_threshold) {
                    // When atleast two tokens from the query are available we
                    // can drop one
                    std::vector<token_t> truncated_tokens;

                    if (orig_tokens.size() > 1 &&
                        num_tokens_dropped < 2 * (orig_tokens.size() - 1)) {
                        bool prefix_search = false;

                        if (num_tokens_dropped < orig_tokens.size() - 1) {
                            // drop from right
                            size_t truncated_len =
                                orig_tokens.size() - num_tokens_dropped - 1;
                            ...
                            size_t start_index = (num_tokens_dropped + 1) -
                                                 orig_tokens.size() + 1;
                            for (size_t i = start_index; i < orig_tokens.size();
                                 i++) {
                                truncated_tokens.emplace_back(orig_tokens[i]);
                            }
                        }
        ...
        do_infix_search(num_search_fields, the_fields, infixes, sort_fields_std,
                        searched_queries, group_limit, group_by_fields,
                        max_extra_prefix, max_extra_suffix,
                        field_query_tokens[0].q_include_tokens, topster,
                        filter_ids, filter_ids_length, sort_order, field_values,
                        geopoint_indices, curated_ids_sorted, all_result_ids,
                        all_result_ids_len, groups_processed);

        /*auto timeMillis0 =
        std::chrono::duration_cast<std::chrono::milliseconds>(
                std::chrono::high_resolution_clock::now() - begin0).count();

        LOG(INFO) << "Time taken for multi-field aggregation: " << timeMillis0
        << "ms";*/
    }
...
process_search_results:
    ...
    if (!facets.empty()) {
        ...
        const size_t window_size =
            (num_threads == 0) ? 0
                               : (all_result_ids_len + num_threads - 1) /
                                     num_threads;  // rounds up
        size_t num_processed = 0;
        std::mutex m_process;
        std::condition_variable cv_process;
        ...
        compute_facet_infos(facets, facet_query, facet_query_num_typos,
                            all_result_ids, all_result_ids_len, group_by_fields,
        ...
        for (size_t thread_id = 0;
             thread_id < num_threads && result_index < all_result_ids_len;
             thread_id++) {
            ...
            thread_pool->enqueue([this, thread_id, &facet_batches, &facet_query,
                                  group_limit, group_by_fields,
                                  batch_result_ids, batch_res_len, &facet_infos,
                                  &num_processed, &m_process, &cv_process]() {
                auto fq = facet_query;
                do_facets(facet_batches[thread_id], fq, facet_infos,
                          group_limit, group_by_fields, batch_result_ids,
                          batch_res_len);
                std::unique_lock<std::mutex> lock(m_process);
                num_processed++;
                cv_process.notify_one();
        ...
        for (auto& facet_batch : facet_batches) {
            for (size_t fi = 0; fi < facet_batch.size(); fi++) {
                ...
                for (auto& facet_kv : this_facet.result_map) {
                    if (group_limit) {
                        ...
                        if (acc_facet.result_map.count(facet_kv.first) == 0) {
                            ...
                            count = acc_facet.result_map[facet_kv.first].count +
                                    facet_kv.second.count;
                        }
                        acc_facet.result_map[facet_kv.first].count = count;
                    }

                    acc_facet.result_map[facet_kv.first].doc_id =
                        facet_kv.second.doc_id;
                    acc_facet.result_map[facet_kv.first].array_pos =
                        facet_kv.second.array_pos;
                    acc_facet.hash_tokens[facet_kv.first] =
                        this_facet.hash_tokens[facet_kv.first];
                }
                ...
                if (this_facet.stats.fvcount != 0) {
                    ...
                    acc_facet.stats.fvmax =
                        std::max(acc_facet.stats.fvmax, this_facet.stats.fvmax);
                    acc_facet.stats.fvmin =
                        std::min(acc_facet.stats.fvmin, this_facet.stats.fvmin);
                }
            }
        }

        /*long long int timeMillisF =
        std::chrono::duration_cast<std::chrono::milliseconds>(
    ...
    do_facets(facets, facet_query, facet_infos, group_limit, group_by_fields,
              &included_ids_vec[0], included_ids_vec.size());
    ...
    // LOG(INFO) << "all_result_ids_len " << all_result_ids_len << " for index "
    // << name; long long int timeMillis =
    // std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now()
    // - begin).count(); LOG(INFO) << "Time taken for result calc: " <<
    // timeMillis << "ms";
}

void Index::process_curated_ids(
    const std::vector<std::pair<uint32_t, uint32_t>>& included_ids,
    const std::vector<uint32_t>& excluded_ids,
    const size_t group_limit,
    const bool filter_curated_hits,
    ...
    // if `filter_curated_hits` is enabled, we will remove curated hits that
    // don't match filter condition
    std::set<uint32_t> included_ids_set;

    if (filter_ids_length != 0 && filter_curated_hits) {
        uint32_t* included_ids_arr = nullptr;
        size_t included_ids_len = ArrayUtils::and_scalar(
            &included_ids_vec[0], included_ids_vec.size(), filter_ids,
        ...
        included_ids_set.insert(included_ids_vec.begin(),
                                included_ids_vec.end());
    }

    std::map<size_t, std::vector<uint32_t>>
        included_ids_grouped;  // pos -> seq_ids
    ...
    if (all_positions.size() > included_ids_map.size()) {
        // Some curated IDs may have been removed via filtering or simply don't
        // exist. We have to shift lower placed hits upwards to fill those
        // positions.
        std::sort(all_positions.begin(), all_positions.end());
        all_positions.erase(unique(all_positions.begin(), all_positions.end()),
                            all_positions.end());
...
void Index::fuzzy_search_fields(const std::vector<search_field_t>& the_fields,
    ...
    // To prevent us from doing ART search repeatedly as we iterate through
    // possible corrections
    spp::sparse_hash_map<std::string, std::vector<std::string>>
        token_cost_cache;

    std::vector<std::vector<int>> token_to_costs;

    for (size_t stoken_index = 0; stoken_index < query_tokens.size();
         stoken_index++) {
        const std::string& token = query_tokens[stoken_index].value;

        std::vector<int> all_costs;
        // This ensures that we don't end up doing a cost of 1 for a single char
        // etc.
        int bounded_cost = get_bounded_typo_cost(2, token.length(),
    ...
    // stores candidates for each token, i.e. i-th index would have all possible
    // tokens with a cost of "c"
    std::vector<tok_candidates> token_candidates_vec;
    ...
    long long n = 0;
    long long int N = token_to_costs.size() > 30
                          ? 1
                          : std::accumulate(token_to_costs.begin(),
                                            token_to_costs.end(), 1LL, product);

    const long long combination_limit = exhaustive_search
                                            ? Index::COMBINATION_MAX_LIMIT
                                            : Index::COMBINATION_MIN_LIMIT;
    ...
    while (n < N && n < combination_limit) {
        ...
        while (token_index < query_tokens.size()) {
            // For each token, look up the generated cost for this iteration and
            // search using that cost
            const std::string& token = query_tokens[token_index].value;
            const std::string token_cost_hash =
                token + std::to_string(costs[token_index]);
            ...
            if (token_cost_cache.count(token_cost_hash) != 0) {
                ...
                for(size_t field_id: field_ids) {
                    ...
                    const bool field_prefix =
                        (the_field.orig_index < prefixes.size())
                            ? prefixes[the_field.orig_index]
                            : prefixes[0];
                    ;
                    const bool prefix_search =
                        field_prefix &&
                        query_tokens[token_index].is_prefix_searched;
                    const size_t token_len = prefix_search
                                                 ? (int)token.length()
                                                 : (int)token.length() + 1;

                    /*LOG(INFO) << "Searching for field: " << the_field.name <<
                       ", token:"
                              << token << " - cost: " << costs[token_index] <<
                       ", prefix_search: " << prefix_search;*/

                    int64_t field_num_typos =
                        (the_field.orig_index < num_typos.size())
                            ? num_typos[the_field.orig_index]
                            : num_typos[0];

                    auto& locale = search_schema.at(the_field.name).locale;
                    if (locale != "" && locale != "en" && locale != "th" &&
                        !Tokenizer::is_cyrillic(locale)) {
                    ...
                    art_fuzzy_search(
                        search_index.at(the_field.name),
                        (const unsigned char*)token.c_str(), token_len,
                        costs[token_index], costs[token_index], max_words,
                        token_order, prefix_search, filter_ids,
                        filter_ids_length, field_leaves, unique_tokens);

                    /*auto timeMillis =
                    std::chrono::duration_cast<std::chrono::milliseconds>(
                                    std::chrono::high_resolution_clock::now() -
                    begin).count(); LOG(INFO) << "Time taken for fuzzy search: "
                    << timeMillis << "ms";*/
            ...
            if(!leaf_tokens.empty()) {
                ...
                // No result at `cost = costs[token_index]`. Remove `cost` for
                // token and re-do combinations
                auto it = std::find(token_to_costs[token_index].begin(),
                                    token_to_costs[token_index].end(),
                                    costs[token_index]);
                if (it != token_to_costs[token_index].end()) {
                    ...
                    if (token_to_costs[token_index].empty()) {
                        // we cannot proceed further, as this token is not found
                        // within cost limits and, dropping of tokens are done
                        // elsewhere.
                        return;
                    }
                }
                ...
                n = -1;
                N = std::accumulate(token_to_costs.begin(),
                                    token_to_costs.end(), 1LL, product);
        ...
        if (token_candidates_vec.size() == query_tokens.size()) {
            ...
            uint32_t* new_all_result_ids = nullptr;
            all_result_ids_len = ArrayUtils::or_scalar(
                all_result_ids, all_result_ids_len, &id_buff[0], id_buff.size(),
                &new_all_result_ids);
            delete[] all_result_ids;
            all_result_ids = new_all_result_ids;
        }
...
void Index::find_across_fields(const token_t& previous_token,
    ...
    result_iter_state_t istate(exclude_token_ids, exclude_token_ids_size,
                               filter_ids, filter_ids_length);
    ...
    for (size_t i = 0; i < num_search_fields; i++) {
        ...
        art_leaf* leaf =
            static_cast<art_leaf*>(art_search(tree, token_c_str, token_len));
        ...
        if (IS_COMPACT_POSTING(leaf->values)) {
            ...
            posting_list_t* full_posting_list =
                compact_posting_list->to_full_posting_list();
            expanded_plists.push_back(full_posting_list);
            its.push_back(full_posting_list->new_iterator(
                nullptr, nullptr, i));  // moved, not copied
        } else {
    ...
    if (its.empty()) {
        // this token does not have any match across *any* field: probably a
        // typo
    ...
    std::sort(
        field_id_doc_counts.begin(), field_id_doc_counts.end(),
    ...
    or_iterator_t::intersect(
        token_its, istate,
        [&](uint32_t seq_id, const std::vector<or_iterator_t>& its) {
...
void Index::search_across_fields(
    const std::vector<token_t>& query_tokens,
    const std::vector<uint32_t>& num_typos,
    const std::vector<bool>& prefixes,
    const std::vector<search_field_t>& the_fields,
    ...
    // one iterator for each token, each underlying iterator contains results of
    // token across multiple fields
    std::vector<or_iterator_t> token_its;
    ...
    for (size_t ti = 0; ti < query_tokens.size(); ti++) {
        ...
        for (size_t i = 0; i < num_search_fields; i++) {
            const std::string& field_name = the_fields[i].name;
            const uint32_t field_num_typos =
                (i < num_typos.size()) ? num_typos[i] : num_typos[0];
            const bool field_prefix =
                (i < prefixes.size()) ? prefixes[i] : prefixes[0];

            if (token_num_typos > field_num_typos) {
                // since the token can come from any field, we still have to
                // respect per-field num_typos
                continue;
            }

            if (token_prefix && !field_prefix) {
                // even though this token is an outcome of prefix search, we
                // can't use it for this field, since this field has prefix
                // search disabled.
                continue;
            }
            ...
            art_leaf* leaf = static_cast<art_leaf*>(
                art_search(tree, token_c_str, token_len));
            ...
            /*LOG(INFO) << "Token: " << token_str << ", field_name: " <<
               field_name
                      << ", num_ids: " << posting_t::num_ids(leaf->values);*/
    ...
    or_iterator_t::intersect(
        ...
        [&](uint32_t seq_id, const std::vector<or_iterator_t>& its) {
            ...
            //  Convert [token -> fields] orientation to [field -> tokens]
            //  orientation
            std::vector<std::vector<posting_list_t::iterator_t>>
                field_to_tokens(num_search_fields);

            for (size_t ti = 0; ti < its.size(); ti++) {
                const or_iterator_t& token_fields_iters = its[ti];
                const std::vector<posting_list_t::iterator_t>& field_iters =
                    token_fields_iters.get_its();

                for (size_t fi = 0; fi < field_iters.size(); fi++) {
                    const posting_list_t::iterator_t& field_iter =
                        field_iters[fi];
                    if (field_iter.id() == seq_id) {
                        // not all fields might contain a given token
                        field_to_tokens[field_iter.get_field_id()].push_back(
                            field_iter.clone());
                    }
                }
            }
            ...
            for (size_t fi = 0; fi < field_to_tokens.size(); fi++) {
                const std::vector<posting_list_t::iterator_t>& token_postings =
                    field_to_tokens[fi];
                ...
                bool field_is_array =
                    search_schema.at(the_fields[fi].name).is_array();
                int64_t field_match_score = 0;

                bool single_exact_query_token = false;
                if (total_cost == 0 && query_tokens.size() == 1) {
                    // does this candidate suggestion token match query token
                    // exactly?
                    single_exact_query_token = true;
                }

                score_results2(sort_fields, searched_queries.size(), fi,
                               field_is_array, total_cost, field_match_score,
                               seq_id, sort_order, prioritize_exact_match,
                               single_exact_query_token,
                               prioritize_token_position, query_tokens.size(),
                               syn_orig_num_tokens, token_postings);
            ...
            compute_sort_scores(
                sort_fields, sort_order, field_values, geopoint_indices, seq_id,
                filter_index, max_field_match_score, scores, match_score_index);
            ...
            // [ sign | tokens_matched | max_field_score | max_field_weight |
            // num_matching_fields ] [   1  |        4       |        48       |
            // 8          |         3           ]  (64 bits)

            auto max_field_weight = std::min<size_t>(
                FIELD_MAX_WEIGHT, the_fields[max_field_match_index].weight);
    ...
    if (!result_ids.empty()) {
        ...
        for (const auto& qtoken : query_tokens) {
            qtoken_set.insert(
                qtoken.value,
                token_leaf(nullptr, qtoken.root_len, qtoken.num_typos,
                           qtoken.is_prefix_searched));
        }
    }
...
void Index::compute_sort_scores(
    const std::vector<sort_by>& sort_fields,
    const int* sort_order,
    std::array<spp::sparse_hash_map<uint32_t, int64_t>*, 3> field_values,
    const std::vector<size_t>& geopoint_indices,
    ...
    for (auto& i : geopoint_indices) {
        ...
        if (geopoints != nullptr) {
            ...
            if (it != field_it->end()) {
                ...
                for (size_t li = 0; li < latlngs[0]; li++) {
                    ...
                    GeoPoint::unpack_lat_lng(packed_latlng, s2_lat_lng);
                    int64_t this_dist =
                        GeoPoint::distance(s2_lat_lng, reference_lat_lng);
                    if (this_dist < dist) {
                        dist = this_dist;
                    }
                }
        ...
        if (sort_fields[i].geo_precision > 0) {
            dist = dist + sort_fields[i].geo_precision - 1 -
                   (dist + sort_fields[i].geo_precision - 1) %
                       sort_fields[i].geo_precision;
        }
    ...
    const int64_t default_score =
        INT64_MIN;  // to handle field that doesn't exist in document (e.g.
                    // optional)
        ...
        if (field_values[0] == &text_match_sentinel_value) {
            ...
            if (scores[0] == adi_tree_t::NOT_FOUND) {
                if (sort_fields[0].order == sort_field_const::asc &&
                    sort_fields[0].missing_values ==
                        sort_by::missing_values_t::first) {
                    scores[0] = -scores[0];
                }

                else if (sort_fields[0].order == sort_field_const::desc &&
                         sort_fields[0].missing_values ==
                             sort_by::missing_values_t::last) {
                    scores[0] = -scores[0];
                }
            }
        } else if (field_values[0] == &eval_sentinel_value) {
            // Returns iterator to the first element that is >= to value or last
            // if no such element is found.
            bool found = false;
            if (filter_index == 0 || filter_index < sort_fields[0].eval.size) {
                size_t found_index =
                    std::lower_bound(
                        sort_fields[0].eval.ids + filter_index,
                        sort_fields[0].eval.ids + sort_fields[0].eval.size,
                        seq_id) -
                    sort_fields[0].eval.ids;

                if (found_index != sort_fields[0].eval.size &&
                    sort_fields[0].eval.ids[found_index] == seq_id) {
            ...
            scores[0] =
                (it == field_values[0]->end()) ? default_score : it->second;

            if (scores[0] == INT64_MIN &&
                sort_fields[0].missing_values ==
                    sort_by::missing_values_t::first) {
                // By default, missing numerical value are always going to be
                // sorted to be at the end because: -INT64_MIN == INT64_MIN. To
                // account for missing values config, we will have to change the
                // default for missing value based on whether it's asc or desc
                // sort.
    ...
    if (sort_fields.size() > 1) {
        if (field_values[1] == &text_match_sentinel_value) {
            ...
            if (filter_index == 0 || filter_index < sort_fields[1].eval.size) {
                size_t found_index =
                    std::lower_bound(
                        sort_fields[1].eval.ids + filter_index,
                        sort_fields[1].eval.ids + sort_fields[1].eval.size,
                        seq_id) -
                    sort_fields[1].eval.ids;

                if (found_index != sort_fields[1].eval.size &&
                    sort_fields[1].eval.ids[found_index] == seq_id) {
                    filter_index = found_index + 1;
                    found = true;
                }
            ...
            scores[1] =
                (it == field_values[1]->end()) ? default_score : it->second;
            if (scores[1] == INT64_MIN &&
                sort_fields[1].missing_values ==
                    sort_by::missing_values_t::first) {
    ...
    if (sort_fields.size() > 2) {
        if (field_values[2] == &text_match_sentinel_value) {
            ...
            if (filter_index == 0 || filter_index < sort_fields[2].eval.size) {
                size_t found_index =
                    std::lower_bound(
                        sort_fields[2].eval.ids + filter_index,
                        sort_fields[2].eval.ids + sort_fields[2].eval.size,
                        seq_id) -
                    sort_fields[2].eval.ids;

                if (found_index != sort_fields[2].eval.size &&
                    sort_fields[2].eval.ids[found_index] == seq_id) {
                    filter_index = found_index + 1;
                    found = true;
                }
            ...
            scores[2] =
                (it == field_values[2]->end()) ? default_score : it->second;
            if (scores[2] == INT64_MIN &&
                sort_fields[2].missing_values ==
                    sort_by::missing_values_t::first) {
...
void Index::do_phrase_search(const size_t num_search_fields,
    ...
    for (size_t i = 0; i < num_search_fields; i++) {
        ...
        for (const auto& phrase : field_query_tokens[i].q_phrases) {
            ...
            for (const std::string& token : phrase) {
                art_leaf* leaf = (art_leaf*)art_search(
                    search_index.at(field_name),
                    (const unsigned char*)token.c_str(), token.size() + 1);
                if (leaf) {
                    posting_lists.push_back(leaf->values);
                }
            }
            ...
            if (posting_lists.size() != phrase.size()) {
                // unmatched length means no matches will be found for this
                // phrase, so skip to next phrase
                continue;
            }
            ...
            posting_t::get_phrase_matches(
                posting_lists, is_array, &contains_ids[0], contains_ids.size(),
                this_phrase_ids, this_phrase_ids_size);

            if (this_phrase_ids_size == 0) {
                // no results found for this phrase, but other phrases can find
                // results
            ...
            if (field_phrase_match_ids_size == 0) {
                ...
                field_phrase_match_ids_size = ArrayUtils::and_scalar(
                    this_phrase_ids, this_phrase_ids_size,
                    field_phrase_match_ids, field_phrase_match_ids_size,
                    &phrase_ids_merged);
        ...
        if (filter_ids_length == 0) {
            ...
            uint32_t* phrase_ids_merged = nullptr;
            phrase_match_ids_size = ArrayUtils::or_scalar(
                phrase_match_ids, phrase_match_ids_size, field_phrase_match_ids,
    ...
    if (filter_ids_length == 0) {
        ...
        filter_ids_length = ArrayUtils::and_scalar(
            filter_ids, filter_ids_length, phrase_match_ids,
...
void Index::do_synonym_search(
    ...
    size_t& all_result_ids_len,
    const uint32_t* filter_ids,
    const uint32_t filter_ids_length,
    std::set<uint64>& query_hashes,
    const int* sort_order,
    std::array<spp::sparse_hash_map<uint32_t, int64_t>*, 3>& field_values,
    const std::vector<size_t>& geopoint_indices,
...
void Index::do_infix_search(
    ...
    for (size_t field_id = 0; field_id < num_search_fields; field_id++) {
        ...
        enable_t field_infix =
            (the_fields[field_id].orig_index < infixes.size())
                ? infixes[the_fields[field_id].orig_index]
                : infixes[0];

        if (field_infix == always ||
            (field_infix == fallback && all_result_ids_len == 0)) {
            std::vector<uint32_t> infix_ids;
            search_infix(query_tokens[0].value, field_name, infix_ids,
                         max_extra_prefix, max_extra_suffix);

            if (!infix_ids.empty()) {
                gfx::timsort(infix_ids.begin(), infix_ids.end());
                infix_ids.erase(std::unique(infix_ids.begin(), infix_ids.end()),
                                infix_ids.end());
                ...
                if (curated_ids_sorted.size() != 0) {
                    raw_infix_ids_length = ArrayUtils::exclude_scalar(
                        &infix_ids[0], infix_ids.size(), &curated_ids_sorted[0],
                ...
                if (filter_ids_length != 0) {
                    ...
                    raw_infix_ids_length = ArrayUtils::and_scalar(
                        filter_ids, filter_ids_length, raw_infix_ids,
                ...
                bool field_is_array =
                    search_schema.at(the_fields[field_id].name).is_array();
                size_t filter_index = 0;
                ...
                for (size_t i = 0; i < raw_infix_ids_length; i++) {
                    ...
                    score_results2(sort_fields, searched_queries.size(),
                                   field_id, field_is_array, 0, match_score,
                                   seq_id, sort_order, false, false, false, 1,
                                   -1, {});

                    int64_t scores[3] = {0};
                    int64_t match_score_index = -1;

                    compute_sort_scores(sort_fields, sort_order, field_values,
                                        geopoint_indices, seq_id, filter_index,
                    ...
                    KV kv(field_id, searched_queries.size(), 0, seq_id,
                          distinct_id, match_score_index, scores);
                ...
                uint32_t* new_all_result_ids = nullptr;
                all_result_ids_len = ArrayUtils::or_scalar(
                    all_result_ids, all_result_ids_len, raw_infix_ids,
...
void Index::handle_exclusion(const size_t num_search_fields,
                             std::vector<query_tokens_t>& field_query_tokens,
                             const std::vector<search_field_t>& search_fields,
                             uint32_t*& exclude_token_ids,
                             size_t& exclude_token_ids_size) const {
    for (size_t i = 0; i < num_search_fields; i++) {
        const std::string& field_name = search_fields[i].name;
        bool is_array = search_schema.at(field_name).is_array();

        for (const auto& q_exclude_phrase :
             field_query_tokens[i].q_exclude_tokens) {
            // if phrase has multiple words, then we have to do exclusion of
            ...
            if (posting_lists.size() == 1) {
                ...
                exclude_token_ids_size = ArrayUtils::or_scalar(
                    exclude_token_ids, exclude_token_ids_size, &contains_ids[0],
                    contains_ids.size(), &exclude_token_ids_merged);
                delete[] exclude_token_ids;
                ...
                posting_t::get_phrase_matches(
                    posting_lists, is_array, &contains_ids[0],
                    contains_ids.size(), phrase_ids, phrase_ids_size);

                uint32_t* exclude_token_ids_merged = nullptr;
                exclude_token_ids_size = ArrayUtils::or_scalar(
                    exclude_token_ids, exclude_token_ids_size, phrase_ids,
                    phrase_ids_size, &exclude_token_ids_merged);
                delete[] phrase_ids;
                delete[] exclude_token_ids;
                exclude_token_ids = exclude_token_ids_merged;
            }
...
void Index::compute_facet_infos(const std::vector<facet>& facets,
                                facet_query_t& facet_query,
                                const size_t facet_query_num_typos,
                                const uint32_t* all_result_ids,
                                const size_t& all_result_ids_len,
                                const std::vector<std::string>& group_by_fields,
                                const size_t max_candidates,
    ...
    for (size_t findex = 0; findex < facets.size(); findex++) {
        ...
        if (a_facet.field_name == facet_query.field_name &&
            ...
            Tokenizer(facet_query.query, true, !facet_field.is_string(),
                      facet_field.locale, symbols_to_index, token_separators)
                .tokenize(query_tokens);

            std::vector<token_t> qtokens;

            for (size_t qtoken_index = 0; qtoken_index < query_tokens.size();
                 qtoken_index++) {
                bool is_prefix = (qtoken_index == query_tokens.size() - 1);
                qtokens.emplace_back(qtoken_index, query_tokens[qtoken_index],
                                     is_prefix,
            ...
            search_field(0, qtokens, nullptr, 0, num_toks_dropped, facet_field,
                         facet_field.faceted_name(), all_result_ids,
                         all_result_ids_len, {}, sort_fields, -1,
                         facet_query_num_typos, searched_queries, topster,
                         groups_processed, &field_result_ids,
                         field_result_ids_len, field_num_results, 0,
                         group_by_fields, false, 4, query_hashes, MAX_SCORE,
                         true, 0, 1, false, -1, 3, 1000, max_candidates);

            // LOG(INFO) << "searched_queries.size: " <<
            // searched_queries.size();

            // NOTE: `field_result_ids` will consists of IDs across ALL queries
            // in searched_queries
            ...
            for (size_t si = 0; si < searched_queries.size(); si++) {
                ...
                for (auto leaf : searched_query) {
                    posting_lists.push_back(leaf->values);
                    std::string tok(reinterpret_cast<char*>(leaf->key),
                                    leaf->key_len - 1);
                ...
                // field_result_ids_len;

                for (size_t i = 0; i < field_result_ids_len; i++) {
                    uint32_t seq_id = field_result_ids[i];

                    const auto doc_fvalues_it =
                        field_facet_mapping_it->second[seq_id % ARRAY_FACET_DIM]
                            ->find(seq_id);
                    if (doc_fvalues_it ==
                        field_facet_mapping_it->second[seq_id % ARRAY_FACET_DIM]
                            ->end()) {
                    ...
                    for (auto pl : posting_lists) {
                        if (!posting_t::contains(pl, seq_id)) {
                            // need to ensure that document ID actually contains
                            // searched_query tokens since `field_result_ids`
                            // contains documents matched across all queries
                            id_matched = false;
                            break;
                        }
                    }
                    ...
                    if (facet_field.is_array()) {
                        ...
                        posting_t::get_matching_array_indices(
                            posting_lists, seq_id, array_indices);

                        for (size_t array_index : array_indices) {
                            if (array_index < doc_fvalues_it->second.length) {
                                uint64_t hash =
                                    doc_fvalues_it->second.hashes[array_index];

                                /*LOG(INFO) << "seq_id: " << seq_id << ", hash:
                                   " << hash << ", array index: "
                                          << array_index;*/

                                if (facet_infos[findex].hashes.count(hash) ==
                                    0) {
                                    facet_infos[findex].hashes.emplace(
                                        hash, searched_tokens);
                                }
                            }
                        }
                    } else {
                        ...
                        if (facet_infos[findex].hashes.count(hash) == 0) {
                            facet_infos[findex].hashes.emplace(hash,
                                                               searched_tokens);
                        }
                    }
                }
...
void Index::curate_filtered_ids(
    ...
    if (!curated_ids.empty()) {
        uint32_t* excluded_result_ids = nullptr;
        filter_ids_length = ArrayUtils::exclude_scalar(
            filter_ids, filter_ids_length, &curated_ids_sorted[0],
            curated_ids_sorted.size(), &excluded_result_ids);
        delete[] filter_ids;
    ...
    if (exclude_token_ids_size != 0) {
        uint32_t* excluded_result_ids = nullptr;
        filter_ids_length = ArrayUtils::exclude_scalar(
            filter_ids, filter_ids_length, exclude_token_ids,
...
void Index::search_wildcard(
    ...
    for (size_t thread_id = 0;
         thread_id < num_threads && filter_index < filter_ids_length;
         thread_id++) {
        size_t batch_res_len = window_size;
        ...
        thread_pool->enqueue(
            [this, &parent_search_begin, &parent_search_stop_ms,
             &parent_search_cutoff, thread_id, &sort_fields, &searched_queries,
             &group_limit, &group_by_fields, &topsters, &tgroups_processed,
             &sort_order, field_values, &geopoint_indices, &plists,
             check_for_circuit_break, batch_result_ids, batch_res_len,
             &num_processed, &m_process, &cv_process]() {
                search_begin = parent_search_begin;
                ...
                for (size_t i = 0; i < batch_res_len; i++) {
                    ...
                    score_results2(sort_fields,
                                   (uint16_t)searched_queries.size(), 0, false,
                                   0, match_score, seq_id, sort_order, false,
                                   false, false, 1, -1, plists);
    ...
    for (size_t thread_id = 0; thread_id < num_processed; thread_id++) {
        groups_processed.insert(tgroups_processed[thread_id].begin(),
                                tgroups_processed[thread_id].end());
    ...
    uint32_t* new_all_result_ids = nullptr;
    all_result_ids_len =
        ArrayUtils::or_scalar(all_result_ids, all_result_ids_len, filter_ids,
                              filter_ids_length, &new_all_result_ids);
    delete[] all_result_ids;
    all_result_ids = new_all_result_ids;
}

void Index::populate_sort_mapping(
    int* sort_order,
    std::vector<size_t>& geopoint_indices,
    std::vector<sort_by>& sort_fields_std,
    std::array<spp::sparse_hash_map<uint32_t, int64_t>*, 3>& field_values)
    ...
    for (size_t i = 0; i < sort_fields_std.size(); i++) {
        ...
        if (sort_fields_std[i].name == sort_field_const::text_match) {
            ...
            if (search_schema.at(sort_fields_std[i].name).type ==
                field_types::GEOPOINT_ARRAY) {
                geopoint_indices.push_back(i);
                field_values[i] =
                    nullptr;  // GEOPOINT_ARRAY uses a multi-valued index
            } else if (search_schema.at(sort_fields_std[i].name).type ==
                       field_types::STRING) {
...
/*
   ...
   2. Outer loop will generate bounded cartesian product with costs for each
   token
   3. Inner loop will iterate on each token with associated cost
   4. Cartesian product of the results of the token searches will be used to
   form search phrases (cartesian product adapted from:
   http://stackoverflow.com/a/31169617/131050)
   4. Intersect the lists to find docs that match each phrase
   5. Sort the docs based on some ranking criteria
*/
void Index::search_field(
    const uint8_t& field_id,
    const std::vector<token_t>& query_tokens,
    const uint32_t* exclude_token_ids,
    size_t exclude_token_ids_size,
    ...
    const std::string& field_name,  // to handle faceted index
    const uint32_t* filter_ids,
    size_t filter_ids_length,
    const std::vector<uint32_t>& curated_ids,
    std::vector<sort_by>& sort_fields,
    ...
    // NOTE: `query_tokens` preserve original tokens, while `search_tokens`
    // could be a result of dropped tokens
    ...
    long long n = 0;
    long long int N = std::accumulate(token_to_costs.begin(),
                                      token_to_costs.end(), 1LL, product);

    const size_t combination_limit = exhaustive_search
                                         ? Index::COMBINATION_MAX_LIMIT
                                         : Index::COMBINATION_MIN_LIMIT;
    ...
    while (n < N && n < combination_limit) {
        ...
        for (long long i = (token_to_costs.size() - 1); 0 <= i; --i) {
            ...
            if (costs[i] == uint32_t(last_typo + 1)) {
                // to support progressive typo searching, there must be atleast
                // one typo that's greater than last_typo
                valid_combo = true;
            }
        }
        ...
        while (token_index < query_tokens.size()) {
            ...
            std::vector<art_leaf*> leaves;
            const bool prefix_search =
                prefix && query_tokens[token_index].is_prefix_searched;
            ...
            if (token_cost_cache.count(token_cost_hash) != 0) {
                ...
                // need less candidates for filtered searches since we already
                // only pick tokens with results
                art_fuzzy_search(search_index.at(field_name),
                                 (const unsigned char*)token.c_str(), token_len,
                                 costs[token_index], costs[token_index],
                                 max_candidates, token_order, prefix_search,
                ...
                LOG(INFO) << "Time taken for fuzzy search: " << timeMillis <<
                "ms";*/
            ...
            if (!leaves.empty()) {
                // log_leaves(costs[token_index], token, leaves);
                token_candidates_vec.push_back(token_candidates{
                    query_tokens[token_index], costs[token_index],
                    prefix_search, leaves});
            }
        ...
        if (token_candidates_vec.size() == query_tokens.size()) {
            ...
            search_candidates(
                field_id, the_field.is_array(), filter_ids, filter_ids_length,
                exclude_token_ids, exclude_token_ids_size, curated_ids,
                sort_fields, token_candidates_vec, searched_queries, topster,
                groups_processed, all_result_ids, all_result_ids_len,
                field_num_results, typo_tokens_threshold, group_limit,
                group_by_fields, query_tokens, prioritize_exact_match,
                exhaustive_search, syn_orig_num_tokens, concurrency,
                query_hashes, id_buff);
...
int Index::get_bounded_typo_cost(const size_t max_cost,
...
void Index::log_leaves(const int cost,
                       const std::string& token,
                       const std::vector<art_leaf*>& leaves) const {
    LOG(INFO) << "Index: " << name << ", token: " << token
              << ", cost: " << cost;

    for (size_t i = 0; i < leaves.size(); i++) {
        std::string key((char*)leaves[i]->key, leaves[i]->key_len);
        LOG(INFO) << key << " - " << posting_t::num_ids(leaves[i]->values);
        LOG(INFO) << "frequency: " << posting_t::num_ids(leaves[i]->values)
                  << ", max_score: " << leaves[i]->max_score;
...
int64_t Index::score_results2(
    ...
    // auto begin = std::chrono::high_resolution_clock::now();
    // const std::string first_token((const char*)query_suggestion[0]->key,
    // query_suggestion[0]->key_len-1);

    if (posting_lists.size() <= 1) {
        const uint8_t is_verbatim_match =
            uint8_t(prioritize_exact_match && single_exact_query_token &&
                    posting_list_t::is_single_token_verbatim_match(
                        posting_lists[0], field_is_array));
        size_t words_present =
            (num_query_tokens == 1 && syn_orig_num_tokens != -1)
                ? syn_orig_num_tokens
                : 1;
        size_t distance = (num_query_tokens == 1 && syn_orig_num_tokens != -1)
                              ? syn_orig_num_tokens - 1
                              : 0;
        size_t max_offset = prioritize_token_position
                                ? posting_list_t::get_last_offset(
                                      posting_lists[0], field_is_array)
                                : 255;
        Match single_token_match =
            Match(words_present, distance, max_offset, is_verbatim_match);
        match_score =
            single_token_match.get_match_score(total_cost, words_present);
        ...
        for (const auto& kv : array_token_positions) {
            ...
            const Match& match =
                Match(seq_id, token_positions, false, prioritize_exact_match);
            uint64_t this_match_score =
                match.get_match_score(total_cost, posting_lists.size());

            // Within a field, only a subset of query tokens can match
            // (unique_words), but even a smaller set might be available within
            // the window used for proximity calculation (this_words_present)

            auto this_words_present = ((this_match_score >> 32) & 0xFF);
            auto unique_words = field_is_array
                                    ? this_words_present
                                    : ((this_match_score >> 40) & 0xFF);
            auto typo_score = ((this_match_score >> 24) & 0xFF);
            auto proximity = ((this_match_score >> 16) & 0xFF);
            auto verbatim = ((this_match_score >> 8) & 0xFF);
            auto offset_score = prioritize_token_position
                                    ? ((this_match_score >> 0) & 0xFF)
                                    : 0;

            if (syn_orig_num_tokens != -1 &&
                num_query_tokens == posting_lists.size()) {
            ...
            uint64_t mod_match_score =
                ((int64_t(this_words_present) << 40) |
                 (int64_t(unique_words) << 32) | (int64_t(typo_score) << 24) |
                 (int64_t(proximity) << 16) | (int64_t(verbatim) << 8) |
    ...
    // std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now()
    // - begin).count(); LOG(INFO) << "Time taken for results iteration: " <<
    // timeNanos << "ms";

    return 0;
}

void Index::score_results(
    const std::vector<sort_by>& sort_fields,
    const uint16_t& query_index,
    const uint8_t& field_id,
    const bool field_is_array,
    ...
    if (posting_lists.size() <= 1) {
        const uint8_t is_verbatim_match =
            uint8_t(prioritize_exact_match && single_exact_query_token &&
                    posting_list_t::is_single_token_verbatim_match(
                        posting_lists[0], field_is_array));
        size_t words_present =
            (syn_orig_num_tokens == -1) ? 1 : syn_orig_num_tokens;
        size_t distance =
            (syn_orig_num_tokens == -1) ? 0 : syn_orig_num_tokens - 1;
        Match single_token_match =
            Match(words_present, distance, is_verbatim_match);
        match_score =
            single_token_match.get_match_score(total_cost, words_present);
    } else {
        std::map<size_t, std::vector<token_positions_t>> array_token_positions;
        ...
        // NOTE: tokens found returned by matcher is only within the best
        // matched window, so we have to still consider unique tokens found if
        ...
        for (const auto& kv : array_token_positions) {
            ...
            const Match& match =
                Match(seq_id, token_positions, false, prioritize_exact_match);
            uint64_t this_match_score =
                match.get_match_score(total_cost, unique_tokens_found);
            ...
            uint64_t mod_match_score =
                ((int64_t(unique_tokens_found) << 32) |
                 ...
                 (int64_t(typo_score) << 16) | (int64_t(proximity) << 8) |
                 (int64_t(verbatim) << 0));
    ...
    KV kv(field_id, query_index, token_bits, seq_id, distinct_id,
          match_score_index, scores);
    topster->add(&kv);
...
uint64_t Index::get_distinct_id(const std::vector<std::string>& group_by_fields,
    ...
    for (const auto& field : group_by_fields) {
        ...
        const auto& field_facet_mapping = field_facet_mapping_it->second;
        const auto& facet_hashes_it =
            field_facet_mapping[seq_id % ARRAY_FACET_DIM]->find(seq_id);

        if (facet_hashes_it ==
            field_facet_mapping[seq_id % ARRAY_FACET_DIM]->end()) {
            continue;
        }
        ...
        for (size_t i = 0; i < facet_hashes.size(); i++) {
            distinct_id =
                StringUtils::hash_combine(distinct_id, facet_hashes.hashes[i]);
        }
    }
...
inline uint32_t Index::next_suggestion2(
    const std::vector<tok_candidates>& token_candidates_vec,
    long long int n,
    ...
    // generate the next combination from `token_leaves` and store it in
    // `query_suggestion`
    ldiv_t q{n, 0};
    for (size_t i = 0; i < token_candidates_vec.size(); i++) {
        ...
        // we assume that toke was found via prefix search if candidate is
        // longer than token's typo tolerance
        bool is_prefix_searched =
            token_candidates_vec[i].prefix_search &&
            (candidate.size() > (token_size + token_candidates_vec[i].cost));

        size_t actual_cost =
            (2 * token_candidates_vec[i].cost) + uint32_t(is_prefix_searched);
        total_cost += actual_cost;

        query_suggestion[i] = token_t(i, candidate, is_prefix_searched,
                                      token_size, token_candidates_vec[i].cost);

        uint64_t this_hash =
            StringUtils::hash_wy(query_suggestion[i].value.c_str(),
                                 query_suggestion[i].value.size());
        qhash = StringUtils::hash_combine(qhash, this_hash);

        /*LOG(INFO) << "suggestion key: " << actual_query_suggestion[i]->key <<
        ", token: "
                  << token_candidates_vec[i].token.value << ", actual_cost: " <<
        actual_cost; LOG(INFO) << ".";*/
    }

    return total_cost;
}

inline uint32_t Index::next_suggestion(
    const std::vector<token_candidates>& token_candidates_vec,
    long long int n,
    std::vector<art_leaf*>& actual_query_suggestion,
    std::vector<art_leaf*>& query_suggestion,
    const int syn_orig_num_tokens,
    ...
    for (long long i = 0; i < (long long)token_candidates_vec.size(); i++) {
        ...
        bool exact_match =
            token_candidates_vec[i].cost == 0 &&
            token_size == actual_query_suggestion[i]->key_len - 1;
        bool incr_for_prefix_search =
            token_candidates_vec[i].prefix_search && !exact_match;

        size_t actual_cost = (2 * token_candidates_vec[i].cost) +
                             uint32_t(incr_for_prefix_search);

        total_cost += actual_cost;

        token_bits |=
            1UL << token_candidates_vec[i].token.position;  // sets n-th bit
...
void Index::remove_facet_token(
    ...
    art_leaf* leaf =
        (art_leaf*)art_search(search_index.at(field_name), key, key_len);
    if (leaf != nullptr) {
        posting_t::erase(leaf->values, seq_id);
        if (posting_t::num_ids(leaf->values) == 0) {
            void* values =
                art_delete(search_index.at(field_name), key, key_len);
            posting_t::destroy_list(values);
        }
    }
}

void Index::remove_field(uint32_t seq_id,
    ...
    // Go through all the field names and find the keys+values so that they can
    // be removed from in-memory index
    if (search_field.type == field_types::STRING_ARRAY ||
        search_field.type == field_types::STRING) {
        std::vector<std::string> tokens;
        tokenize_string_field(document, search_field, tokens,
                              search_field.locale, symbols_to_index,
                              token_separators);
        ...
        const std::vector<int32_t>& values =
            search_field.is_single_integer()
        ...
        for (int32_t value : values) {
            ...
            if (search_field.facet) {
                remove_facet_token(search_field, search_index,
                                   std::to_string(value), seq_id);
            }
        }
        ...
        for (float value : values) {
            ...
            if (search_field.facet) {
                remove_facet_token(search_field, search_index,
                                   StringUtils::float_to_str(value), seq_id);
            }
        }
        ...
        const std::vector<std::vector<double>>& latlongs =
            search_field.is_single_geopoint()
                ? std::vector<
                      std::vector<double>>{document[field_name]
        ...
        for (const std::vector<double>& latlong : latlongs) {
            ...
            for (const auto& term : indexer.GetIndexTerms(point, "")) {
                ...
                ids.erase(std::remove(ids.begin(), ids.end(), seq_id),
                          ids.end());
                if (ids.empty()) {
        ...
        if (!search_field.is_single_geopoint()) {
            spp::sparse_hash_map<uint32_t, int64_t*>*& field_geo_array_map =
                geo_array_index.at(field_name);
    ...
    if (field_facets_it != facet_index_v3.end()) {
        const auto& fvalues_it =
            field_facets_it->second[seq_id % ARRAY_FACET_DIM]->find(seq_id);
        if (fvalues_it !=
            field_facets_it->second[seq_id % ARRAY_FACET_DIM]->end()) {
            field_facets_it->second[seq_id % ARRAY_FACET_DIM]->erase(
                fvalues_it);
        }
    }
...
Option<uint32_t> Index::remove(const uint32_t seq_id,
                               const nlohmann::json& document,
                               const std::vector<field>& del_fields,
                               const bool is_update) {
    std::unique_lock lock(mutex);

    // The exception during removal is mostly because of an edge case with auto
    // schema detection: Value indexed as Type T but later if field is dropped
    // and reindexed in another type X, the on-disk data will differ from the
    // newly detected type on schema. We've to log the error, but have to ignore
    // the field and proceed because there's no leak caused here.
    ...
    if (!del_fields.empty()) {
        for (auto& the_field : del_fields) {
            ...
            try {
                ...
                LOG(WARNING) << "Error while removing field `" << the_field.name
                             << "` from document, message: " << e.what();
            }
        }
    } else {
        for (auto it = document.begin(); it != document.end(); ++it) {
            ...
            try {
                ...
                LOG(WARNING) << "Error while removing field `" << field_name
                             << "` from document, message: " << e.what();
            }
        }
    }
...
void Index::tokenize_string_field(const nlohmann::json& document,
                                  const field& search_field,
                                  std::vector<std::string>& tokens,
    ...
    if (search_field.type == field_types::STRING) {
        Tokenizer(document[field_name], true, false, locale, symbols_to_index,
                  token_separators)
            .tokenize(tokens);
    } else if (search_field.type == field_types::STRING_ARRAY) {
        const std::vector<std::string>& values =
            document[field_name].get<std::vector<std::string>>();
        for (const std::string& value : values) {
            Tokenizer(value, true, false, locale, symbols_to_index,
                      token_separators)
                .tokenize(tokens);
        }
    }
...
art_leaf* Index::get_token_leaf(const std::string& field_name,
...
const spp::sparse_hash_map<std::string, art_tree*>& Index::_get_search_index()
    const {
    return search_index;
}

const spp::sparse_hash_map<std::string, num_tree_t*>&
Index::_get_numerical_index() const {
    return numerical_index;
}

const spp::sparse_hash_map<std::string, array_mapped_infix_t>&
Index::_get_infix_index() const {
    return infix_index;
};
...
void Index::refresh_schemas(const std::vector<field>& new_fields,
    ...
    for (const auto& new_field : new_fields) {
        ...
        if (search_index.count(new_field.name) == 0) {
            if (new_field.is_string() ||
                field_types::is_string_or_array(new_field.type)) {
                art_tree* t = new art_tree;
                art_tree_init(t);
                search_index.emplace(new_field.name, t);
            } else if (new_field.is_geopoint()) {
                auto field_geo_index =
                    new spp::sparse_hash_map<std::string,
                                             std::vector<uint32_t>>();
        ...
        if (new_field.type == field_types::FLOAT_ARRAY && new_field.num_dim) {
            auto hnsw_index =
                new hnsw_index_t(new_field.num_dim, 1024, new_field.vec_dist);
    ...
    for (const auto& del_field : del_fields) {
        ...
        if (del_field.is_string() ||
            field_types::is_string_or_array(del_field.type)) {
            ...
            if (!del_field.is_single_geopoint()) {
                spp::sparse_hash_map<uint32_t, int64_t*>* geo_array_map =
                    geo_array_index[del_field.name];
...
Option<uint32_t> Index::coerce_string(const DIRTY_VALUES& dirty_values,
                                      const std::string& fallback_field_type,
                                      const field& a_field,
                                      nlohmann::json& document,
                                      const std::string& field_name,
    ...
    if (dirty_values == DIRTY_VALUES::REJECT) {
        return Option<>(
            400, "Field `" + field_name + "` must be " + suffix + " string.");
    }

    if (dirty_values == DIRTY_VALUES::DROP) {
        if (!a_field.optional) {
            return Option<>(400, "Field `" + field_name + "` must be " +
                                     suffix + " string.");
        }
...
Option<uint32_t> Index::coerce_int32_t(const DIRTY_VALUES& dirty_values,
                                       const field& a_field,
                                       nlohmann::json& document,
                                       const std::string& field_name,
                                       nlohmann::json::iterator& array_iter,
    ...
    if (dirty_values == DIRTY_VALUES::REJECT) {
        return Option<>(
            400, "Field `" + field_name + "` must be " + suffix + " int32.");
    }
    ...
    if (document.contains(field_name) &&
        document[field_name].get<int64_t>() > INT32_MAX) {
        if (a_field.optional &&
            (dirty_values == DIRTY_VALUES::DROP ||
             dirty_values == DIRTY_VALUES::COERCE_OR_REJECT)) {
            document.erase(field_name);
        } else {
            return Option<>(400, "Field `" + field_name +
                                     "` exceeds maximum value of int32.");
        }
    }
...
Option<uint32_t> Index::coerce_int64_t(const DIRTY_VALUES& dirty_values,
                                       const field& a_field,
                                       nlohmann::json& document,
                                       const std::string& field_name,
                                       nlohmann::json::iterator& array_iter,
    ...
    if (dirty_values == DIRTY_VALUES::REJECT) {
        return Option<>(
            400, "Field `" + field_name + "` must be " + suffix + " int64.");
    }
...
Option<uint32_t> Index::coerce_bool(const DIRTY_VALUES& dirty_values,
                                    const field& a_field,
                                    nlohmann::json& document,
                                    const std::string& field_name,
                                    nlohmann::json::iterator& array_iter,
    ...
    if (dirty_values == DIRTY_VALUES::REJECT) {
        return Option<>(
            400, "Field `" + field_name + "` must be " + suffix + " bool.");
    }
...
Option<uint32_t> Index::coerce_geopoint(const DIRTY_VALUES& dirty_values,
                                        const field& a_field,
                                        nlohmann::json& document,
                                        const std::string& field_name,
                                        nlohmann::json::iterator& array_iter,
    ...
    if (dirty_values == DIRTY_VALUES::REJECT) {
        return Option<>(
            400, "Field `" + field_name + "` must be " + suffix + " geopoint.");
    }
...
Option<uint32_t> Index::coerce_float(const DIRTY_VALUES& dirty_values,
                                     const field& a_field,
                                     nlohmann::json& document,
                                     const std::string& field_name,
                                     nlohmann::json::iterator& array_iter,
    ...
    if (dirty_values == DIRTY_VALUES::REJECT) {
        return Option<>(
            400, "Field `" + field_name + "` must be " + suffix + " float.");
    }
...
void Index::get_doc_changes(const index_operation_t op,
    ...
    for (; it != update_doc.end();) {
        // if the update doc contains a field that exists in old, we record that
        // (for delete + reindex)
...
void Index::scrub_reindex_doc(const tsl::htrie_map<char, field>& search_schema,
    ...
    for (auto it = del_doc.cbegin(); it != del_doc.cend(); it++) {
        ...
        const auto search_field =
            search_field_it.value();  // copy, don't use reference!
        ...
        if (update_doc.contains(search_field.name)) {
            if (update_doc[search_field.name].is_null()) {
                // we don't allow null values to be stored or indexed but need
                // to be removed from stored doc
                update_doc.erase(search_field.name);
            } else if (update_doc[search_field.name] ==
                       old_doc[search_field.name]) {
...
void Index::resolve_space_as_typos(
    ...
    for (const std::string& token : qtokens) {
        art_leaf* leaf = (art_leaf*)art_search(
            t, (const unsigned char*)token.c_str(), token.length() + 1);
        if (leaf == nullptr) {
            break;
        }
    ...
    // When we cannot find verbatim match, we can try concatting and splitting
    // query tokens for alternatives.

    // Concatenation:

    size_t qtokens_size = std::min<size_t>(
        5, qtokens.size());  // only first 5 tokens will be considered

    if (qtokens.size() > 1) {
        // a) join all tokens to form a single string
        const string& all_tokens_query = StringUtils::join(qtokens, "");
        if (art_search(t, (const unsigned char*)all_tokens_query.c_str(),
                       all_tokens_query.length() + 1) != nullptr) {
            resolved_queries.push_back({all_tokens_query});
            return;
        }

        // b) join 2 adjacent tokens in a sliding window (provided they are
        // atleast 2 tokens in size)
        ...
        for (size_t i = 0; i < qtokens_size - 1 && qtokens_size > 2; i++) {
            ...
            for (auto& token : candidate_tokens) {
                art_leaf* leaf = static_cast<art_leaf*>(
                    art_search(t, (const unsigned char*)token.c_str(),
                               token.length() + 1));
                if (leaf == nullptr) {
                    break;
                }
            ...
            if (candidate_tokens.size() == leaves.size() &&
                common_results_exist(leaves, false)) {
    ...
    for (size_t i = 0; i < qtokens_size; i++) {
        ...
        for (size_t ci = 1; ci < token.size(); ci++) {
            ...
            art_leaf* first_leaf = static_cast<art_leaf*>(
                art_search(t, (const unsigned char*)first_part.c_str(),
                           first_part.length() + 1));
            ...
            if (first_leaf != nullptr) {
                ...
                art_leaf* second_leaf = static_cast<art_leaf*>(
                    art_search(t, (const unsigned char*)second_part.c_str(),
                               second_part.length() + 1));

                std::vector<art_leaf*> part_leaves = {first_leaf, second_leaf};
                if (second_leaf != nullptr &&
                    common_results_exist(part_leaves, true)) {
        ...
        for (auto& candidate_token : candidate_tokens) {
            art_leaf* leaf = static_cast<art_leaf*>(
                art_search(t, (const unsigned char*)candidate_token.c_str(),
...
bool Index::common_results_exist(std::vector<art_leaf*>& leaves,
    ...
    posting_t::get_phrase_matches(leaf_vals, false, &result_ids[0],
                                  result_ids.size(), phrase_ids,
                                  num_phrase_ids);
...
// NOTE: polygon and point should have been transformed with
`transform_for_180th_meridian` bool Index::is_point_in_polygon(const Geofence&
poly, const GeoCoord &point) { int i, j; bool c = false;

    for (i = 0, j = poly.numVerts - 1; i < poly.numVerts; j = i++) {
        if ((((poly.verts[i].lat <= point.lat) && (point.lat <
poly.verts[j].lat))
             || ((poly.verts[j].lat <= point.lat) && (point.lat <
poly.verts[i].lat)))
            && (point.lon < (poly.verts[j].lon - poly.verts[i].lon) * (point.lat
- poly.verts[i].lat) / (poly.verts[j].lat - poly.verts[i].lat) +
poly.verts[i].lon)) {
