    ...
    if (f.is_integer()) {
        ...
        for (size_t fi = 0; fi < a_filter.values.size(); fi++) {
            ...
            if (a_filter.comparators[fi] == RANGE_INCLUSIVE &&
                ...
                const std::string& next_filter_value = a_filter.values[fi + 1];
                int64_t range_end_value = float_to_int64_t(
        ...
        // aggregates IDs across array of filter values and reduces excessive
        // ORing
        ...
        for (const std::string& filter_value : a_filter.values) {
            ...
            if (a_filter.comparators[0] == EQUALS ||
                ...
                posting_t::get_exact_matches(
                    posting_lists, f.is_array(), result_id_vec.data(),
                    result_id_vec.size(), exact_str_ids, exact_str_ids_size);
            ...
            if (f_id_buff.size() > 100000 || a_filter.values.size() == 1) {
                ...
                f_id_buff.erase(std::unique(f_id_buff.begin(), f_id_buff.end()),
                                f_id_buff.end());

                uint32_t* out = nullptr;
                or_ids_size =
                    ArrayUtils::or_scalar(or_ids, or_ids_size, f_id_buff.data(),
                                          f_id_buff.size(), &out);
                delete[] or_ids;
                or_ids = out;
        ...
        if (!f_id_buff.empty()) {
            ...
            uint32_t* out = nullptr;
            or_ids_size = ArrayUtils::or_scalar(
                or_ids, or_ids_size, f_id_buff.data(), f_id_buff.size(), &out);
            delete[] or_ids;
            or_ids = out;
            std::vector<uint32_t>().swap(f_id_buff);  // clears out memory
        }
        ...
        if (a_filter.comparators[0] == NOT_EQUALS) {
            // exclude records from existing IDs (from previous filters or ALL
            // records) "not equals" can only be applied to the entire array so
            // we can do the exclusion operations once here
            ...
            excluded_strt_size =
                ArrayUtils::exclude_scalar(result_ids, result_ids_len, or_ids,
