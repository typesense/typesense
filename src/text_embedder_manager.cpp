...
Option<bool> TextEmbedderManager::validate_and_init_remote_model(const nlohmann::json& model_config,
    ...
    auto text_embedder_it = text_embedders.find(is_remote_model(model_name) ? RemoteEmbedder::get_model_key(model_config) : model_name);
    if(text_embedder_it == text_embedders.end()) {
        text_embedders.emplace(is_remote_model(model_name) ? RemoteEmbedder::get_model_key(model_config) : model_name, std::make_shared<TextEmbedder>(model_config, num_dims));
    }
