...
void Join::aggregate_nested_references(single_filter_result_t *const reference_result,
...
void Join::negate_left_join(id_list_t* const seq_ids, std::unique_ptr<uint32_t[]>& reference_docs, uint32_t& reference_docs_count,
...
void Join::get_ref_index_ids(num_tree_t& ref_index, const uint32_t& reference_doc_id,
...
void Join::get_ref_index_ids(const spp::sparse_hash_map<uint32_t, int64_t, Hasher32>& ref_index,
...
void Join::do_nested_join(std::function<std::pair<size_t, uint32_t*>(const uint32_t& ref_doc_id)> const& get_related_ids,
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        auto pair = get_related_ids(reference_doc_id);
        size_t& doc_ids_len = pair.first;
        uint32_t*& doc_ids = pair.second;
...
void Join::process_related_ids(std::vector<std::pair<uint32_t, uint32_t>> id_pairs, const size_t& unique_doc_ids_size,
