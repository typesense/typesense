...
Option<bool> ConversationModel::validate_model(const nlohmann::json& model_config) {
    ...
    if(model_config.count("history_collection") != 0) {
        if(!model_config["history_collection"].is_string()) {
            return Option<bool>(400, "Property `history_collection` is not a string.");
        }
