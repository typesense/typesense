...
Option<bool> EventManager::add_event(const nlohmann::json& event, const std::string& client_ip) {
    ...
    */
    ...
    if(event_type_val.is_string()) {
        ...
        if(event_type == "search") {
        ...
        } else if(event_type == "query_click" || event_type == "query_special") {
            if (!event.contains("data")) {
                return Option<bool>(404, "key `data` not found.");
            }
            ...
            auto op = AnalyticsManager::get_instance().add_click_special_event(event_type, collection, query, user_id, doc_id, position, client_ip);
            if(!op.ok()) {
