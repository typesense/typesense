...
bool post_multi_search_agg(const std::shared_ptr<http_req>& req, const std::shared_ptr<http_res>& res) {
    ...
    LOG(INFO) << "req_json = " << req_json.dump();

    for(const auto& query : req_json.items()) {
        if(query.key() != "collections") {
            AuthManager::add_item_to_params(req->params, query, true);
        } else {
            // concat query array into string
            std::string collections;
            for(const auto& collection : query.value()) {
                collections += collection.get<std::string>() + ",";
            }

            // remove last comma
            collections.pop_back();

            req->params["collections"] = collections;
        }
    }
    ...
    std::string results_json_str;
    Option<bool> search_op = CollectionManager::do_search(req->params, req->embedded_params_vec[0], results_json_str, true);
