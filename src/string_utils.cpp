...
bool StringUtils::split_ref_include_params(const std::string& params, std::map<std::string, std::string>& result) {
    ...
    for(const auto& tok : tokens) {
        ...
        if(kv_tokens[0] == ref_include::strategy_key || kv_tokens[0] == ref_include::related_docs_count
            || kv_tokens[0] == ref_include::limit) {
            result.emplace(kv_tokens[0], kv_tokens[1]);
            ...
            result.emplace(kv_tokens[0], val);
        } else {
            ...
            if(result.count(ref_include::sort_by)) {
                auto str = kv_tokens[0] + ":" + kv_tokens[1];
                result[ref_include::sort_by] += (std::string(", ") + str);
            } else {
                result.emplace(kv_tokens[0], kv_tokens[1]);
            }
        }
