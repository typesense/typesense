#include "raft_coordinator.h"
#include "raft_server.h"
#include "raft_node_manager.h"
#include <logger.h>
#include <braft/raft.h>

RaftCoordinator::RaftCoordinator(HttpServer* server,
    ...
    LOG(INFO) << "Creating RaftCoordinator components";
    ...
    LOG(INFO) << "RaftCoordinator initialized with clean component separation";
}
...
int RaftCoordinator::start(const butil::EndPoint& peering_endpoint,
                          int api_port,
    ...
    LOG(INFO) << "Starting RaftCoordinator";
    ...
    LOG(INFO) << "RaftCoordinator started successfully";
    return 0;
}

void RaftCoordinator::shutdown() {
    if (state_machine) {
        LOG(INFO) << "Shutting down RaftCoordinator";
    ...
    LOG(INFO) << "RaftCoordinator shutdown complete";
}
