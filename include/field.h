...
struct field
{
    ...
    field(const std::string &name, const std::string &type, const bool facet, const bool optional = false,
          ...
          int nested_array = 0, size_t num_dim = 0, vector_distance_type_t vec_dist = cosine) : name(name), type(type), facet(facet), optional(optional), index(index), locale(locale),
...
struct sort_by
{
    ...
    sort_by(const std::string &name, const std::string &order) : name(name), order(order), text_match_buckets(0), geopoint(0), exclude_radius(0), geo_precision(0),
                                                                 missing_values(normal)
    {
    }

    sort_by(const std::string &name, const std::string &order, uint32_t text_match_buckets, int64_t geopoint,
            uint32_t exclude_radius, uint32_t geo_precision) : name(name), order(order), text_match_buckets(text_match_buckets),
