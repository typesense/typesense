...
private:
    ...
    rocksdb::Status init_db(int32_t ttl) {
        LOG(INFO) << "Initializing Analytics DB by opening state dir: " << state_dir_path;

        rocksdb::Status s = rocksdb::DBWithTTL::Open(options, state_dir_path, &db, ttl);
...
public:
    ...
    StoreWithTTL(const std::string & state_dir_path,
          const size_t wal_ttl_secs = 24*60*60,
          const size_t wal_size_mb = 1024, bool disable_wal = true, int32_t ttl=FOURWEEKS_SECS): state_dir_path(state_dir_path) {
        // Optimize RocksDB
        options.IncreaseParallelism();
        options.OptimizeLevelStyleCompaction();
        // create the DB if it's not already present
    ...
    Option<std::vector<std::string>*> get_updates_since(const uint64_t seq_number_org, const uint64_t max_updates) const {
        ...
        std::unique_ptr<rocksdb::TransactionLogIterator> iter;
        rocksdb::TransactionLogIterator::ReadOptions readOptions;
        rocksdb::Status status = db->GetUpdatesSince(seq_number, &iter, readOptions);
        if(!status.ok()) {
            LOG(ERROR) << "Error while fetching updates for replication: " << status.ToString();
